...
아래는 `context_lookup`으로 근거를 가져오는 대표 흐름이다. MCP는 Codex나 Claude 같은 AI host가 검색 서버의 도구를 호출하고 결과를 받는 연결 규약이다.

```mermaid
flowchart TD
    A["커밋된 Markdown<br/>지식 원문"] --> B["제목, 별칭, 절, 링크 추출"]
    B --> C["그래프 색인<br/>문서와 절의 연결, 원문 위치"]
    Q["사용자 질문"] --> H["AI host가 MCP 도구 호출"]
    H --> D["검색 후보를 찾고 순위 결정"]
    C --> D
...
현재 검색 서버는 키워드와 원문에 명시된 관계를 사용한다. 임베딩이나 벡터 DB는 사용하지 않으며, 답변 생성은 Codex나 Claude 같은 AI host가 담당한다. `source_confirmed`는 원문에서 연결을 확인했다는 뜻이다. 내용의 정확성이나 현재 프로젝트에 대한 적용 가능성은 별도로 판단한다.
...
이 순서도는 2026-09-15 Codex 세션에서 확인한 사례다. 수정한 코드를 자동으로 다시 읽지 않는 것은 서버 구현의 성질이므로 Claude에서도 서버 프로세스를 새로 시작해야 새 코드가 적용된다.
