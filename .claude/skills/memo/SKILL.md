---
name: memo
description: 사용자가 강의/세미나/학습 메모나 블로그, 아티클을 정리해달라고 할 때 사용. "메모 정리", "강의 내용 정리", "세미나 내용 정리", "학습 내용 정리", "블로그 정리", "아티클 정리", "영상 정리", "유튜브 정리", "이거 정리해줘" 등의 요청에 트리거. 텍스트 붙여넣기뿐 아니라 URL, YouTube 영상과 녹음 파일 입력도 지원.
...
4. 예상 대상이 `biz/`나 `econ/`이면 해당 도메인의 `CLAUDE.md`를 읽는다.
...
- YouTube 영상 URL이나 로컬 오디오, 영상 파일은 `.claude/skills/memo/scripts/yt_transcript.py <URL 또는 파일> --out <저장소 밖 출력 폴더>`로 전사한다. 성공하면 종료 코드 0과 함께 마지막 줄에 `<출력 폴더>/<작업 폴더>/transcript.md` 경로를 출력하며, 종료 코드가 0이 아니면 오류 메시지가 본문 전사 경로를 알려 준 경우 말고는 폴더에 남은 파일을 결과로 쓰지 않는다. 전사문은 문서 작성의 입력일 뿐 vault에 저장하지 않는다.
...
출처와 관련 문서는 문서 하단에 이 순서로 둔다. tech 문서의 표기는 `tech/CLAUDE.md`의 출처 표기 규칙을 따른다. 아래는 그 규칙이 정의한, 선례가 없는 새 tech 문서의 기본형 템플릿이다.
