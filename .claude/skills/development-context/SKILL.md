...
2. 결과의 `index_sync`, `coverage_gaps`, 예산과 검사하지 않은 항목을 먼저 확인한다. `matching.assessment`가 `weak_lexical_overlap`이면 긴 질문과 같은 section에서 겹치는 단어가 매우 적다는 신호다. 반환된 자료가 질문을 실제로 다루는지 확인하고, 무관하면 표현이나 scope를 바꿔 다시 찾는다. 이 표시는 의미 적합성이나 지식 부재의 판정이 아니다. 근거는 반환된 커밋의 원문이다. dirty 또는 revision 불일치가 있으면 현재 작업 파일과 혼합하지 않고 커밋과 현재 파일의 차이를 확인한다.
...
5. 필요한 문서가 조회 결과에 없거나 root 제한으로 후보가 잘렸으면 `context_search`로 문서 후보를 찾는다. 같은 `query`, 주제에 맞는 `scope`, `max_bytes: 24000`으로 시작하고 관련 문서를 찾으면 목차나 원문을 읽는다. 첫 페이지로 부족하면 같은 query와 scope에 `pagination.next_cursor`를 `cursor`로 전달해 한 페이지 더 탐색한다. 두 페이지 뒤에도 부족하면 표현이나 scope를 조정한다. `cursor_mismatch`는 검색을 처음부터 다시 시작하고, 예산 부족은 최대 65,536 byte까지 늘린다. `candidates`와 `matched_terms`는 문서 metadata와 여러 section의 어휘 겹침이며 본문 근거나 의미적 적합성 점수가 아니다. `best_evidence_ref`도 원문 읽기의 시작점이므로 해당 본문에 모든 검색어가 있다고 가정하지 않는다. 도구가 없으면 [문서 검색 절차](../../../ontology/Ontology-Document-Search.md)의 CLI를 사용한다. 반환된 revision과 색인 상태를 확인하고 이전 revision의 페이지와 섞지 않는다.
6. 관련 Document는 찾았지만 필요한 heading이나 다른 조건이 빠졌으면 `context_outline`로 전체 목차를 확인한다. Document의 `id`를 `document_id`로, 같은 조회의 `index_sync` revision을 `source_revision`으로 전달한다. `offset_sections: 0`, `max_bytes: 24000`에서 시작해 `pagination.complete`까지 `next_offset_sections`로 이어 간다. 응답 예산 부족은 최대 65,536 byte까지 늘리고, revision이 바뀌면 재조회한다. 도구가 없으면 [목차 조회 절차](../../../ontology/Ontology-Document-Outline.md)의 CLI 또는 같은 revision의 원문을 읽는다. 목차는 본문 근거가 아니므로 질문에 필요한 section을 골라 본문을 확인한다.
7. 검색 요약이나 지도에만 의존하지 않고 선택한 원문 section과 예외 조건을 읽는다. 목차에서 고른 section 또는 `truncated: true`인 중요한 발췌가 있고 `context_read`가 보이면 조회 결과의 `id`를 `evidence_unit_id`로, `source_revision`과 `content_hash`는 그대로 전달한다. `offset_bytes: 0`, `max_bytes: 24000`에서 시작하고 `pagination.complete`가 true가 될 때까지 반환된 `next_offset_bytes`로 이어 읽는다. `budget_too_small`이면 같은 시작 위치에서 예산을 최대 65,536 byte까지 늘려 다시 읽는다. revision 또는 hash 불일치가 나면 다시 조회하고 처음부터 읽으며 이전 페이지와 합치지 않는다. 도구가 없으면 [근거 읽기 절차](../../../ontology/Ontology-Evidence-Read.md)의 CLI 또는 반환된 revision, path와 byte anchor의 Git 원문을 사용한다. 동명 heading은 전체 heading 경로, occurrence와 byte anchor로 구분한다. 현재 작업 파일로 보완했다면 pinned 원문과의 차이를 확인한다. 파일이나 heading이 사라졌거나 잘린 조건을 더 읽을 수 없으면 근거 부족으로 남긴다.
