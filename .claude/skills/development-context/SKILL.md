...
5. 검색 요약이나 지도에만 의존하지 않고 선택한 원문 section과 예외 조건을 읽는다. 파일이나 heading이 사라졌으면 이동 여부를 확인하고, 해결되지 않으면 근거 부족으로 남긴다.
...
기록이 허용된 작업에서는 채택된 결정, 이유와 적용 범위를 해당 프로젝트의 정본에 남긴다. 기술 정정은 원문을 고치고, 조회 누락은 실제 사례를 근거로 지도를 보강한다. 미채택 제안은 결정으로 저장하지 않는다.
