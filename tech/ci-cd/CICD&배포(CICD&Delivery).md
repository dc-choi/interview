...
- [x] [[CICD-Basics|CI/CD 기초 (CI/Delivery/Deployment, 배포 두려움과 미루기, 파이프라인 단계, 트리거, 성숙도 0~5)]]
...
- [x] [[git|Git 폴더 인덱스 (멘탈 모델, 머지 전략, reset/reflog 복구)]]
- [x] [[Git-Mental-Model|Git 멘탈 모델 (커밋 스냅샷/브랜치 포인터/HEAD, fast-forward, 3-way merge, rebase 원리)]]
- [x] [[Git-Merge-Strategies|Git 통합 방식 (Merge commit/Squash/Rebase, fast-forward와 히스토리 재작성)]]
- [x] [[Git-Reset-Reflog|Git Reset과 복구 (reset/revert, reflog, force-with-lease, range-diff)]]
- [x] [[GitHub-Actions]]
- [x] [[Single-Host-SPA-API-Deployment|단일 서버 SPA/API 배포 (Nginx, TLS, GitHub Actions, 환경 변수, CORS, rollback)]]
