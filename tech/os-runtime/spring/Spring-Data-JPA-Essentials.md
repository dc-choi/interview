...
| 최상위 (편의) | **Spring Data JPA** | **JPA 명세 위의 Repository 추상화**. Repository 인터페이스 자동 구현. 특정 구현체가 아니라 JPA 명세에 의존 |
...
### 영속성 컨텍스트가 주는 이점
1차 캐시, Dirty Checking, 쓰기 지연, Lazy Loading — 성능과 코드 단순성 동시 확보 (→ [[JPA-Persistence-Context]]). 단 쓰기 지연은 만능이 아니라 키 전략에 종속된다. `IDENTITY`는 ID 확보를 위해 `persist()` 시점에 INSERT가 바로 나가 지연이 깨지고, `SEQUENCE`는 미리 ID를 받아 지연이 유지된다 (아래 [ID 생성 전략](#id-생성-전략) 참고).
