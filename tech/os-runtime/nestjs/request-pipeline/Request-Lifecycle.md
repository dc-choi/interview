...
### 1. Middleware
- Express 미들웨어와 유사
- 전역(`app.use`) 뒤에 모듈에 바인딩한 미들웨어가 실행. 여러 모듈이면 루트 모듈부터 imports 배열 순서
- 요청과 응답 객체에 직접 접근 가능
...
- 전역 => 컨트롤러 => 라우팅 메서드 => 파라미터 순으로 작동
- 컨트롤러와 라우팅 메서드에 바인딩한 파이프는 마지막 파라미터부터 첫 파라미터 순으로 실행되고, 파라미터 전용 파이프는 그 뒤에 같은 순서로 실행됩니다.
