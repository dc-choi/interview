...
- **내장 HTTP 예외는 로그되지 않는다**: `HttpException`과 내장 서브클래스는 `IntrinsicException`을 상속해 정상 애플리케이션 흐름과 구분된다. 이를 로그하려면 커스텀 HTTP 필터가 필요하다. WebSocket과 RPC 예외의 로깅 정책은 각 전송의 필터에서 따로 정한다.
...
- **Microservice(RPC) 계층 차이**: `RpcException`(@nestjs/microservices)을 던지면 `{ status: 'error', message }` 에러 객체로 응답된다. RPC 필터는 한 가지가 다르다 — **catch()가 Observable을 반환해야** 한다 (보통 `throwError(() => exception.getError())`). 코어 확장은 `BaseRpcExceptionFilter`. 하이브리드 앱에서는 전역 microservice 필터가 기본으로 활성화되지 않는다.
