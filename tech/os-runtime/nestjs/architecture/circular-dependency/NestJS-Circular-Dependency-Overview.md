...
- forwardRef로 Nest의 인스턴스화는 우회할 수 있지만 코드가 읽기 어려워지고, 도메인 경계가 흐려진다.
...
| **ModuleRef Lazy** | 인스턴스 사후 해결 | 동적 토큰, 조건부 의존 | 생성자 DI 간선이 숨겨져 정적 분석과 테스트 부담 증가 |
...
- `Nest cannot create the <module> instance. The module at index [N] of the <module> "imports" array is undefined.` — **모듈 레벨 순환**의 전형적 메시지 (또는 import 문 오류로 정말 undefined가 들어간 경우). 모듈 순환은 양쪽 `imports`에 `forwardRef()`를 쓰고, 별도의 프로바이더 순환도 있을 때만 양쪽 생성자 주입에도 적용한다.
- `Nest can't resolve dependencies ... argument dependency at index ...` — 누락된 provider/export, 잘못된 토큰 외에 **파일 순환 import**도 흔한 원인. 모듈 파일에서 상수를 export하고 서비스가 그걸 도로 import하는 패턴이 단골이므로, 상수는 별도 파일로 분리한다. 판독 전체는 [[Custom-Provider|Cannot resolve dependency 진단]].
