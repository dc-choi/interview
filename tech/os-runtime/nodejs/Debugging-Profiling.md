...
| 응답 시간이 느림 (높은 레이턴시) | V8 프로파일러, Linux Perf | 프로파일링 |
| 메모리 사용량이 지속적으로 증가 | Heap Snapshot, GC Traces | 메모리 진단 |
| CPU 사용량이 높음 | Flame Graph, Perf | Flame Graph |
| 프로세스 충돌/재시작 반복 | Heap Snapshot + GC Traces | 메모리 진단 |
