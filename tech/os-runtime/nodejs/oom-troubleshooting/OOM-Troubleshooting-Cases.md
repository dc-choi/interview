...
- **범위 해석:** MySQL `max_prepared_stmt_count`는 서버 전역 상한이고, mysql2 LRU만 커넥션별이다. 커넥션 수를 전역 상한에 곱하거나 구문당 메모리를 고정 32KB로 두지 않고 `Prepared_stmt_count`, 프로세스 heap, 서버 메모리를 함께 관측했다.
