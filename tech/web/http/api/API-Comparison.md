...
- **GraphQL**: 단일 엔드포인트에 POST를 주로 쓰면 REST의 자원 URL 기반 캐시를 그대로 쓰기 어려움. 서버가 조회 연산의 GET을 지원하면 HTTP 캐시를 활용할 수 있고, 긴 쿼리는 persisted document로 짧은 식별자만 보내 CDN 캐싱을 보완할 수 있음. 객체 id를 키로 쓰는 클라이언트 정규화 캐시는 별도 축
