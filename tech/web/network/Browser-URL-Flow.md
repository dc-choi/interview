...
3. **라우터/ISP 리졸버** — 재귀 리졸버(ISP 또는 8.8.8.8). 집 공유기가 DNS로 잡혀 있으면 공유기가 **DNS 포워더**로 동작해 외부 리졸버에 대신 질의
...
- **GSLB** — 지리, 서버 상태, 부하, 네트워크 품질을 종합해 최적 리전의 IP를 응답. 단순 DNS 라운드로빈과는 목적과 깊이가 다름 ([[Load-Balancer|Load Balancer, GSLB]])
- **헬스 체크와 페일오버** — 서울 리전 장애 시 해당 IP를 응답에서 제외하고 다른 리전으로 유도. 사용자에게는 전체 장애가 아니라 일시적 지연
- **세션 유지** — 장애, 분산으로 다른 서버에 붙어도 로그인이 유지되어야 함 → 세션 외부 저장소(Redis) 분리 또는 JWT ([[Auth-Method-Selection|인증 방식 선택]])
- **보안** — DNS 스푸핑(가짜 서버 유도 → 피싱, 계정 탈취)과 DDoS. 방어는 CDN, WAF, LB, GSLB 조합
- **무중단 배포** — 블루-그린, 롤링, 카나리로 배포 안정성과 장애 대응을 함께 설계 ([[Blue-Green]])
