---
tags: [web, network]
status: index
category: "Web - 네트워크"
aliases: ["Network"]
---

# 네트워크(Network)

OSI 7계층·TLS·브라우저 URL 입력부터 렌더링까지.

## 목차
- [x] [[HTTPS-TLS|HTTPS / TLS Handshake]]
- [x] [[OSI-7-Layer|OSI 7계층과 Internet vs Ethernet]]
- [x] [[Physical-DataLink-Layer|물리와 데이터링크 계층 (L1/L2 — 허브, 스위치, 충돌 도메인, CSMA/CD, MAC, 프레임)]]
- [x] [[TCP-Handshake|TCP Handshake (3-way·4-way, TIME_WAIT, HTTPS 왕복 비용)]]
- [x] [[Loopback-And-Localhost|Loopback · Localhost 동작 원리 (127.0.0.1·::1, loopback NIC, 커널 내 처리, 디버깅)]]
- [x] [[Browser-URL-Flow|브라우저 URL 입력 프로세스 (DNS→ARP→TCP/TLS→HTTP→렌더링, Core Web Vitals)]]
