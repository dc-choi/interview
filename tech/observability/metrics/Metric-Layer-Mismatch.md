...
측정 레이어가 같아도 아래 조건이 다르면 수치와 알람 시각을 바로 비교할 수 없다.

| 차원 | 확인 질문 |
|---|---|
| 모집단 | 같은 host, service, operation과 사용자 집합인가 |
| 분자와 분모 | 오류 수와 오류율처럼 서로 다른 값을 비교하고 있지 않은가 |
| 집계 | sum, average, max와 percentile 중 무엇인가 |
| 평가 창 | 최근 5분과 최근 30분처럼 다른 구간인가 |
| 해상도 | 원본 interval과 rollup 주기가 같은가 |
| 시간 경계 | timezone과 bucket 시작점이 같은가 |
| 누락 처리 | sampling, `No Data`, 0 채움과 지연 수집 정책이 같은가 |

### 알람 발화 순서는 사건 순서가 아니다

같은 시계열도 짧은 평가 창과 낮은 임계값을 쓴 알람이 먼저 울린다. 따라서 "A 알람이 B보다 먼저 발화했다"는 사실만으로 A가 원인이라고 결론 내리지 않는다. 원시 시계열의 실제 변곡점을 같은 모집단, 창과 해상도로 다시 맞춘 뒤 인과 가설을 세운다.
