...
`maxmemory-policy = allkeys-lru`는 TTL 여부와 무관하게 모든 키 중 근사 LRU 기준으로 삭제한다.
TTL이 있는 키만 삭제 대상으로 삼는 것은 `volatile-lru`, `volatile-lfu`, `volatile-ttl` 같은 `volatile-*` 정책이다.
...
- Redis 7.4+의 **Hash 필드 단위 TTL**(`HEXPIRE`)로 필드별 만료. 운영 중인 Redis, Valkey, ElastiCache 버전 지원 여부를 확인해야 한다.
