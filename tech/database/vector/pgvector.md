...
| 타입 | 정밀도 | 차원당 | 최대 차원 |
|------|--------|--------|-----------|
| `vector` | 32비트 float | 4바이트 | 2,000 |
| `halfvec` | 16비트 float | **2바이트** | 4,000 |

`halfvec`은 저장 공간이 절반이라 메모리, I/O 비용을 줄인다. **검색 품질 차이가 크지 않다면 halfvec이 유리**하다. 단 도입 전 실제 임베딩 값이 halfvec의 유효 정밀도 안에서 충분히 표현되는지(품질 저하가 없는지) 검증해야 한다.

## 인덱스는 HNSW — 동적 데이터에 적합

서비스 데이터는 계속 추가/변경되므로, 재구성 없이 그래프에 삽입 가능한 **HNSW**가 실무 기본이다. `m`, `ef_construction`, `ef_search`는 기본값(16/64/40)으로 출발해도 대부분 충분하고, 필요할 때 조정한다.

## 대용량 테이블은 쪼갠다

HNSW 인덱스는 그래프 이웃을 따라 탐색하므로 **B-tree보다 쿼리당 읽는 페이지가 많다**. 한 테이블에 수천만 건이 몰리면 큰 인덱스를 버퍼 캐시에 다 올리기 어렵고 랜덤 I/O가 늘어 검색과 삽입 성능이 같이 떨어진다.

- **분할**: PostgreSQL 파티셔닝(지역 기준 등)이나 카테고리별 물리 테이블 분리로 **쿼리가 탐색할 범위를 줄인다**.
- **왜 중요한가**: 벡터 인덱스는 **일반 컬럼과 복합 인덱스를 만들 수 없다.** 그래서 일반 인덱스라면 복합 인덱스로 했을 사전 필터링을 **파티셔닝(파티션 프루닝)**으로 대신해야 효과가 크다.
...
- 동적 데이터에 HNSW를 쓰는 이유, 기본값에서 출발하는 전략
- 벡터 인덱스가 복합 인덱스 불가 → 파티셔닝으로 사전 필터링하는 설계
...
- [[OpenSearch|OpenSearch kNN]] — 검색 엔진 쪽 벡터 검색 대안
