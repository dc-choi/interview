...
Prisma의 NestJS 배선은 **PrismaClient를 상속한 @Injectable PrismaService 하나로 감싸는 패턴**이 공식이다 — `.prisma` 스키마 DSL에서 코드 생성된 타입 클라이언트를 DI로 주입해 서비스가 `this.prisma.user.findMany()`처럼 쓴다 (전용 통합 패키지 없이 커스텀 프로바이더 한 개로 충분한 구조).

대안으로 MikroORM도 있다 — **Data Mapper + Unit of Work + Identity Map 패턴** 기반이라 아키텍처가 다르다(변경을 모아 flush 시점에 쓰기, 같은 요청 안 동일 엔티티는 동일 인스턴스 보장). 이 설계 때문에 요청마다 깨끗한 상태가 필요해서, NestJS 통합(`@mikro-orm/nestjs`, 서드파티 유지)은 RequestContext 미들웨어로 **요청별 EntityManager fork**를 자동 처리한다 (HTTP 요청 밖에서는 `@CreateRequestContext()` 데코레이터로 컨텍스트 생성).
