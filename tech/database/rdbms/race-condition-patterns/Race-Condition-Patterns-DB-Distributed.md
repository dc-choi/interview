...
**선택**: 품목 행에 `SELECT ... FOR UPDATE NOWAIT`를 걸어 대기 대신 즉시 실패시키고, 애플리케이션에서 100ms 간격으로 최대 3회 재시도했다. 재시도 대기 합이 최대 300ms 수준이라 디바이스 타임아웃 1초 안에 수락과 거절이 결정된다. 디바이스 조회와 요청 검증은 트랜잭션 밖으로 빼 트랜잭션 안에는 재고 행 잠금과 갱신만 남겼다.
