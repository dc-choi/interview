...
→ 따라서 **앱 컴퓨팅보다 DB의 확장 비용이 큰 경우가 많다**는 경향을 전제로, DB 병목을 다른 계층으로 옮길 수 있는지 검토한다.
...
모든 로직을 앱에 넣으면 안 된다. 다음은 DB가 더 적합한 영역이다.
...
## ORM과 Stored Procedure 선택

ORM은 객체와 테이블의 변환을 맡겨 애플리케이션 코드에서 도메인 로직을 다루게 한다. 단순 CRUD, 빠른 변경, 애플리케이션 테스트와 배포 일원화가 중요한 경우에 유리하다. 반면 N+1, 불필요한 컬럼 조회와 비효율적인 생성 SQL을 그대로 두면 생산성이 DB 비용으로 전가된다.

예를 들어 주문 목록을 한 번 조회한 뒤 각 주문의 lazy-loaded 사용자에 접근하면 주문 수만큼 추가 쿼리가 실행되어 1+N 경로가 된다. 필요한 연관을 fetch join으로 한 번에 가져오거나 batch loading으로 `IN` 쿼리를 묶어 왕복 수를 제한한다. 자세한 내용은 [[JPA-Persistence-Context|JPA 영속성 컨텍스트와 N+1]]을 참고한다.

Stored Procedure는 데이터가 있는 DB 안에서 여러 연산을 한 번에 수행한다. 대량 배치, 복잡한 집계와 여러 SQL 왕복이 실제 병목인 구간에서는 네트워크 왕복을 줄이고 DB의 집합 연산을 활용할 수 있다. 대신 다음 비용을 함께 감수한다.

- DB 제품별 언어와 기능에 묶여 이식 비용이 커진다.
- 애플리케이션과 별도의 테스트, 배포, 관측 경로가 필요하다.
- 계산이 DB CPU에 집중되므로 확장 한계가 더 빨리 올 수 있다.
- 즉시 반영이 쉽다는 이유로 검토와 롤백 절차를 생략하면 변경 이력이 불명확해진다.

선택할 때는 기술 특성뿐 아니라 팀의 SQL 역량, 전담 DB 조직 유무, 애플리케이션과 DB 변경의 배포 및 운영 책임 구조도 함께 본다.

기본 CRUD와 비즈니스 규칙은 ORM과 애플리케이션 계층에 두고, 측정으로 확인된 대량 처리와 집계 병목에는 네이티브 SQL이나 Stored Procedure를 선택적으로 사용한다. Stored Procedure도 migration script로 버전 관리하고, 통합 테스트와 실행 시간, 오류율을 애플리케이션 배포와 같은 수준으로 관리한다.
...
- **모든 로직을 SP에 집중** — 팀 변경, 기술 교체와 DB 확장 시 비용 급증
...
- ORM과 Stored Procedure를 선택하는 기준과 혼합 전략
