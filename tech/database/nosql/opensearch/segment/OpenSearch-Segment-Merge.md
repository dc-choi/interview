...
| floor segment | 16MB (OpenSearch 3.x, Lucene 10.2+) | 이보다 작은 segment는 같은 tier로 취급해 적극 merge |
| `max_merged_segment` | 5GB | 자동 merge가 만드는 segment 크기 상한 |

Floor segment는 잦은 refresh가 만든 초소형 segment가 tier 계산을 오염시키지 않게 바닥 크기로 묶는 장치다. Lucene 10.2부터 기본값이 2MB에서 16MB로 바뀌었으므로 OpenSearch 2.x 이하를 운영한다면 해당 버전의 소스와 실제 index 설정을 확인한다. 반대쪽 상한인 5GB가 shard 크기 권장과 연결된다. 자동 merge는 5GB보다 큰 segment를 만들지 않으므로, shard가 수백 GB면 5GB급 segment가 수십 개 쌓여 검색당 순회할 segment 수와 복구, 재배치 단위가 함께 커진다. AWS가 검색 latency 중심 workload에 shard당 10~30GiB, 로그 분석에 30~50GiB를 권장하는 것은 이 상한과 heap당 shard 수 제약을 함께 반영한 값이다.
...
- delete하면 디스크가 바로 준다 → 아니다. 삭제 표시만 남고, merge가 그 segment를 재작성해야 회수된다. `deletes_pct_allowed=20`은 문서 수 비율이므로 byte overhead 20%를 보장하거나 예측하지 않는다.
...
- [OpenSearch Documentation, Index codecs](https://docs.opensearch.org/latest/im-plugin/index-codecs/)
