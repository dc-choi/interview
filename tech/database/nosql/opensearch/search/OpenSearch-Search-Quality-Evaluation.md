...
GET products/_rank_eval
{
  ...
  "metric": { "dcg": { "k": 10, "normalize": true, "unknown_doc_rating": 0 } }
}
```
...
| dcg | `k=10`, `normalize=false`, `unknown_doc_rating`은 선택 | 등급과 순위 가중, `normalize=true`가 nDCG |
...
`ignore_unlabeled`는 precision metric의 파라미터다. nDCG@10과 MRR을 재현하려면 Rank Evaluation을 두 번 실행해 `dcg: {k: 10, normalize: true, unknown_doc_rating: 0}`와 `mean_reciprocal_rank: {k: 10, relevant_rating_threshold: 1}`를 각각 고정한다. 작은 corpus는 query-document 쌍을 전수 판정하고 `details.<queryId>.unrated_docs` 배열 길이와 `details.<queryId>.metric_details.dcg.unrated_docs` 숫자가 모두 0인지 확인한다. 큰 corpus에서 retrieval 결과만 합친 pool은 모든 arm이 함께 놓친 관련 문서를 누락할 수 있으므로, 비교 arm과 독립된 pool 구성과 coverage 한계를 함께 기록한다.

### 반환 수가 적을 때의 false-green

OpenSearch 3.6.0 core의 normalized DCG는 IDCG 길이를 `k`가 아니라 실제 반환 hit 수에 맞춰 자른다. `k=10`이어도 relevant 문서 하나만 1위로 반환하면 API nDCG와 MRR이 모두 1이 될 수 있다. `details.<queryId>.unrated_docs=[]`, `details.<queryId>.metric_details.dcg.unrated_docs=0`과 zero-result 검사도 나머지 relevant 문서의 누락을 찾지 못한다. 다른 version에서는 해당 tag의 core source와 REST 응답으로 이 동작을 다시 검증한다.
...
Relevant 문서가 하나도 없는 query는 nDCG와 MRR이 0이므로 ranking 평균에 섞으면 no-match query 비율이 모델 점수를 바꾼다. 이런 query는 명시적으로 no-match 집합에 두고 ranking metric의 query 수와 분모에는 relevant 문서가 하나 이상인 query만 포함한다.

첫 전수 fixture의 no-match 정책은 `STRICT_EMPTY`로 고정한다. `no_match_false_positive_query_rate = returned_count가 1 이상인 query 수 / no-match query 수`로 계산하고 0만 통과시킨다. 제품이 교정, 제안이나 fallback을 의도한다면 원 검색 결과와 `responseMode`를 분리하고 별도 label, 성공률과 guardrail로 평가한다. Ranking 실패를 fallback 성공으로 가리지 않는다.

## UBI: 행동 데이터 수집

Open-source OpenSearch의 User Behavior Insights plugin(2.15+)은 검색 행동을 표준 schema로 수집한다.
...
Amazon OpenSearch Service에는 native UBI plugin이 포함되지 않는다. 관리형에서는 애플리케이션이나 Amazon OpenSearch Ingestion이 UBI schema의 `ubi_queries`, `ubi_events`를 직접 적재하는 대체 수집 경로를 구성해야 한다. 같은 인덱스 이름을 사용해도 native plugin의 server-side 자동 수집이 생기는 것은 아니다.
...
Pointwise metric 집합과 relevance threshold는 version에 종속된다. Workbench 3.1 도입 문서는 `Coverage@k`, `Precision@k`, `MAP@k`, `NDCG@k`를 설명하고, upstream OpenSearch 3.6부터 `Recall@k`, `MRR`, `DCG@k`가 추가됐다. 3.6 구현은 judgment 분포에서 계산한 `T = max(0.5 * J_max, P90)`을 `Precision@k`, `Recall@k`, `MAP@k`, `MRR`의 binary relevance threshold로 함께 사용한다. `DCG@k`와 `NDCG@k`는 graded relevance를 사용해 이 threshold의 영향을 받지 않는다. 따라서 고정 `relevant_rating_threshold`를 쓰는 core Rank Evaluation 결과와 정의를 맞추지 않은 채 직접 비교하지 않고, engine/plugin version, threshold 규칙과 실제 출력 metric 목록을 결과에 함께 저장한다. Coverage@k는 반환 문서 중 judgment가 있는 비율로, 이 값이 낮으면 나머지 지표를 신뢰할 수 없다는 신호다.

## 온라인 지표

오프라인 지표가 좋아져도 사용자가 못 느끼면 개선이 아니다. UBI 데이터 위에서 최소 세 가지를 추이로 본다.

- CTR: 검색당 클릭 비율. 위치, snippet, UI와 click propensity가 섞인 행동 지표이므로 관련도 자체로 해석하지 않는다.
...
- CTR과 Click-MRR 상승만으로 관련도 개선을 확정하면 안 된다. UI, snippet, 위치와 selection bias가 섞이므로 randomization이나 propensity 보정 없이 causal label로 사용할 수 없다.
...
- [[Recommendation-System-Online-Experimentation-Statistics|검색과 추천 온라인 실험 통계]]

## 출처

- [OpenSearch Documentation, Ranking evaluation](https://docs.opensearch.org/latest/api-reference/search-apis/rank-eval/)
- [Rank Evaluation 응답과 DCG — OpenSearch 3.6.0 core](https://github.com/opensearch-project/OpenSearch/blob/3.6.0/modules/rank-eval/src/main/java/org/opensearch/index/rankeval/RankEvalResponse.java), [EvalQueryQuality.java](https://github.com/opensearch-project/OpenSearch/blob/3.6.0/modules/rank-eval/src/main/java/org/opensearch/index/rankeval/EvalQueryQuality.java), [DiscountedCumulativeGain.java](https://github.com/opensearch-project/OpenSearch/blob/3.6.0/modules/rank-eval/src/main/java/org/opensearch/index/rankeval/DiscountedCumulativeGain.java)
- [MeanReciprocalRank.java — OpenSearch 3.6.0 core](https://github.com/opensearch-project/OpenSearch/blob/3.6.0/modules/rank-eval/src/main/java/org/opensearch/index/rankeval/MeanReciprocalRank.java)
- [PrecisionAtK.java — OpenSearch 3.6.0 core](https://github.com/opensearch-project/OpenSearch/blob/3.6.0/modules/rank-eval/src/main/java/org/opensearch/index/rankeval/PrecisionAtK.java)
- [OpenSearch Documentation, User Behavior Insights](https://docs.opensearch.org/latest/search-plugins/ubi/index/)
- [OpenSearch Documentation, Collecting UBI-formatted data in Amazon OpenSearch Service](https://docs.opensearch.org/latest/search-plugins/ubi/ubi-aws-managed-services-tutorial/)
- [OpenSearch Documentation, UBI index schemas](https://docs.opensearch.org/latest/search-plugins/ubi/schemas/)
- [OpenSearch Documentation, Search Relevance Workbench](https://docs.opensearch.org/latest/search-plugins/search-relevance/using-search-relevance-workbench/), [OpenSearch 3.6.0 release notes](https://github.com/opensearch-project/opensearch-build/blob/main/release-notes/opensearch-release-notes-3.6.0.md), [EvaluationMetrics.java 3.6.0.0](https://github.com/opensearch-project/search-relevance/blob/3.6.0.0/src/main/java/org/opensearch/searchrelevance/metrics/EvaluationMetrics.java), [JudgmentThresholdCalculator.java 3.6.0.0](https://github.com/opensearch-project/search-relevance/blob/3.6.0.0/src/main/java/org/opensearch/searchrelevance/metrics/calculator/JudgmentThresholdCalculator.java)
- [OpenSearch Documentation, Query sets](https://docs.opensearch.org/latest/search-plugins/search-relevance/query-sets/)
- [OpenSearch Documentation, Judgments](https://docs.opensearch.org/latest/search-plugins/search-relevance/judgments/)
- [OpenSearch Documentation, Evaluating search quality](https://docs.opensearch.org/latest/search-plugins/search-relevance/evaluate-search-quality/)
