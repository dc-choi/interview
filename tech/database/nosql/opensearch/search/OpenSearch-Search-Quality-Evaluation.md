...
`ignore_unlabeled`는 precision metric의 파라미터다. nDCG@10과 MRR을 재현하려면 Rank Evaluation을 두 번 실행해 `dcg: {k: 10, normalize: true, unknown_doc_rating: 0}`와 `mean_reciprocal_rank: {k: 10, relevant_rating_threshold: 1}`를 각각 고정한다. 작은 corpus는 query-document 쌍을 전수 판정하고 `unrated_docs=0`을 확인한다. 큰 corpus에서 retrieval 결과만 합친 pool은 모든 arm이 함께 놓친 관련 문서를 누락할 수 있으므로, 비교 arm과 독립된 pool 구성과 coverage 한계를 함께 기록한다.

Relevant 문서가 하나도 없는 query는 nDCG와 MRR이 0이므로 ranking 평균에 섞으면 no-match query 비율이 모델 점수를 바꾼다. 이런 query는 명시적으로 no-match 집합에 두고 처리 성공률을 별도 집계하며, nDCG/MRR의 query 수와 분모에는 relevant 문서가 하나 이상인 query만 포함한다.
...
- [DiscountedCumulativeGain.java - OpenSearch core](https://github.com/opensearch-project/OpenSearch/blob/main/modules/rank-eval/src/main/java/org/opensearch/index/rankeval/DiscountedCumulativeGain.java)
- [MeanReciprocalRank.java - OpenSearch core](https://github.com/opensearch-project/OpenSearch/blob/main/modules/rank-eval/src/main/java/org/opensearch/index/rankeval/MeanReciprocalRank.java)
- [PrecisionAtK.java - OpenSearch core](https://github.com/opensearch-project/OpenSearch/blob/main/modules/rank-eval/src/main/java/org/opensearch/index/rankeval/PrecisionAtK.java)
