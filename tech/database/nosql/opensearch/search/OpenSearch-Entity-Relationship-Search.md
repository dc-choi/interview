...
역정규화가 join 비용을 쓰기로 옮긴다는 트레이드오프 자체는 [[OpenSearch-vs-RDB-Search#검색엔진 도입의 대가|검색엔진 도입의 대가]]가 정본이다. 이때 비용이 사라지는 것은 아니다. 동의어 방식에서 사전 배포에 숨어 있던 갱신 비용이, 버전 관리와 검증이 가능한 색인 파이프라인으로 옮겨온다.
