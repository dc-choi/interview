...
사용자 사전은 의미가 같은 단어를 연결하지 않고 token 경계만 바꾼다. 동의어는 경계를 고치지 않는다. `C++` 같은 표현은 형태소 사용자 사전에만 맡기지 말고 정규화된 `keyword` field나 별도 exact field를 함께 검토한다. 동의어는 같은 개체나 개념의 표기 변형에만 쓴다. 인물과 출연작처럼 서로 다른 개체 사이의 관계는 [[OpenSearch-Entity-Relationship-Search|개체 관계 검색 모델링]]대로 문서 필드로 푼다.
...
- [OpenSearch Documentation, Term vectors](https://docs.opensearch.org/latest/api-reference/document-apis/termvector/)
...
- [OpenSearch Documentation, Refresh search analyzer](https://docs.opensearch.org/latest/im-plugin/refresh-analyzer/)
- [OpenSearch Documentation, Additional plugins](https://docs.opensearch.org/latest/install-and-configure/additional-plugins/)
...
- [AWS Documentation, OpenSearch Service 지원 plugin](https://docs.aws.amazon.com/opensearch-service/latest/developerguide/supported-plugins.html), [AWS Documentation, OpenSearch Serverless 지원 plugin](https://docs.aws.amazon.com/opensearch-service/latest/developerguide/serverless-genref.html)
- [AWS Documentation, OpenSearch Service custom package](https://docs.aws.amazon.com/opensearch-service/latest/developerguide/custom-packages.html)
