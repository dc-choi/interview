...
```mermaid
flowchart LR
  BK["Bulk 색인 요청"] --> PS["Primary shard"]
  PS --> TL["Translog fsync<br/>장애 복구용 내구성"]
  PS --> BUF["In-memory buffer"]
  PS -->|"복제"| RS["Replica shard"]
  BUF -->|"refresh (기본 1초)"| SEG["새 segment<br/>여기서부터 검색 가능"]
  classDef plain fill:#E9EDF3,stroke:#8A94A3,color:#1B2430;
  classDef os fill:#1E63C4,stroke:#17509E,color:#FFFFFF;
  class BK,PS,BUF,RS,TL plain;
  class SEG os;
```

색인 응답 성공(translog fsync, 내구성)과 검색에 보임(refresh, 가시성)은 다른 경계다. 저장 직후 검색에 안 보이는 게 버그가 아니라 설계인 이유. 그림 1의 read-after-write 분기가 여기서 나온다.
