---
tags: [ai, context-engineering, context-bottleneck, claude-md, productivity]
status: done
...
> 이 vault는 Claude Code auto memory를 사용하지 않는다. 지속 규칙은 루트 또는 가장 가까운 도메인의 `AGENTS.md`에 두고, 같은 경로의 `CLAUDE.md`는 `@AGENTS.md` import 브리지로만 사용한다. 반복 절차는 Skills에 둔다.

## 지시 파일 설계

Anthropic은 각 `CLAUDE.md`를 200줄보다 짧게 유지하는 것을 목표로 제안한다. 이는 제품의 하드 제한이 아니라 중요한 지시가 묻히지 않도록 하는 작성 지침이다.

- 루트 `AGENTS.md`: 저장소 전체에 적용되는 불변 규칙과 도메인 라우팅
- 중첩 `AGENTS.md`: 해당 경로에서만 유지할 불변 규칙
- 각 `CLAUDE.md`: 같은 경로의 `AGENTS.md`를 import하는 Claude Code 호환 브리지
...
1. 루트에는 전역 불변 규칙만 두고 경로별 규칙을 가까운 `AGENTS.md`로 내린다. `CLAUDE.md`에는 규칙을 복제하지 않는다.
