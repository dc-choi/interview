---
tags: [ai, llm, prompt-caching, cost, bedrock]
status: done
...
코딩 에이전트 구현에서 도구 등록 순서를 고정하는 이유도 같다 — 순서가 바뀌면 시스템 프롬프트가 달라져 캐시가 무효화된다 ([[Claude-Code-Internals]]). 단, Claude Opus 5부터는 베타 대화 중 도구 변경으로 캐시를 유지한 채 도구 셋을 바꿀 수 있다 ([[Claude-Opus-5|Claude Opus 5]]).
