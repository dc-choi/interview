---
tags: [ai, llm, claude, api, availability]
status: done
...
거부는 4xx/5xx 예외가 아니라 정상 응답의 한 종료 사유다. 따라서 에러 핸들러가 아니라 응답 분기 로직에서 처리한다. 기존 통합이 `stop_reason`을 `end_turn`, `tool_use` 정도만 분기했다면 `refusal` 케이스를 새로 추가해야 한다.
