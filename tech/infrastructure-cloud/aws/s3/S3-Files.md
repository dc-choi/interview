...
- 1 MiB씩 10회 읽기 → S3 direct 경로 → S3 Files의 파일 읽기 요금은 없고, S3 GET 요청과 4 KiB 메타데이터 읽기는 청구
...
- **S3 게이트웨이 엔드포인트**는 별도 시간당 요금이나 데이터 처리 요금이 없으며 콜드 읽기의 NAT Gateway 데이터 처리 비용을 피할 수 있다. 2026년 4월부터 5월까지 서울 리전에서 측정한 공식 AWS 블로그 예시는 1 GB 콜드 읽기에 엔드포인트 경유 $0, NAT 경유 $0.045, S3에 도달할 경로가 없을 때 고성능 스토리지 대체 읽기 $0.030을 기록했다. 실제 도입 시에는 리전별 최신 S3 Files, NAT Gateway, S3 요청 요금을 다시 계산한다 ([[VPC|VPC Gateway Endpoint]])
...
- 게이트웨이 엔드포인트가 프라이빗 서브넷의 NAT Gateway 처리 비용을 줄이는 이유
...
- [Amazon S3 Files 동기화 동작](https://docs.aws.amazon.com/ko_kr/AmazonS3/latest/userguide/s3-files-synchronization.html)
