...
기본 3계층(CloudWatch 인프라 지표, Database Insights DB 부하, 로그)과 Slow Query 알람은 [[RDS-Monitoring]]에서 다룬다. 여기서는 Aurora 장애를 깊게 파고드는 지표, 인스턴스 이벤트 알림 파이프라인, CloudWatch로 부족한 부분을 메우는 커스텀 관측을 다룬다. 모니터링의 목적은 무슨 일이 일어났는지(로그), 얼마나 심각한지(알림), 어디부터 봐야 하는지(지표, 대시보드)에 답하는 것이다.
...
- binlog I/O cache는 Aurora MySQL v2.10부터 도입된 기능이다. 그러나 Aurora MySQL v2는 표준 지원이 종료됐으므로 신규 또는 업그레이드 대상은 호환성 검증 뒤 현재 지원되는 v3 이상이나 조건에 맞는 Extended Support minor를 선택한다. 기능 도입 버전과 현행 권장 버전을 혼동하지 않는다.
...
## Database Insights용 부하 데이터는 장애 전에 수집

어떤 SQL, 호스트, 사용자, 대기 이벤트가 부하를 만드는지 보여준다. **장애가 난 뒤 켜면 과거 원인 분석이 어렵다.** Performance Insights 독립 콘솔은 2026-07-31 종료되므로, 핵심 서비스 DB는 Database Insights Standard 또는 Advanced mode와 retention을 미리 선택한다. Performance Insights API는 유지된다(상세: [[RDS-Monitoring-Metrics#Database Insights와 Performance Insights 전환]]).
...
- **AWS가 맡는 것**: 서비스가 약속한 범위의 스토리지 내구성, 관리형 복구 메커니즘, 자동 백업 인프라와 사용자가 구성한 Auto Scaling 정책의 실행.
- **운영자가 맡는 것**: Reader 생성 또는 Aurora Auto Scaling의 지표, 최소, 최대 용량 구성, AZ별 컴퓨트 배치([[RDS-Aurora-Architecture#컴퓨트 노드 AZ 배치]]), 엔진과 버전 선택, [[RDS-Aurora-Graviton|Graviton 전환 검증]], 알람 임계치, 로그 분석, 비용 관리 자동화.

관측을 새로 깔 때의 실천 순서:

1. CloudWatch 알람과 RDS 이벤트 알림(Event Subscription)을 먼저 만든다.
2. Database Insights mode와 retention을 정하고 slow query 로그를 켠다.
...
- [AWS Aurora User Guide, Amazon CloudWatch metrics for Amazon Aurora](https://docs.aws.amazon.com/AmazonRDS/latest/AuroraUserGuide/Aurora.AuroraMySQL.Monitoring.Metrics.html)
...
- [AWS RDS User Guide, Master user account privileges](https://docs.aws.amazon.com/AmazonRDS/latest/UserGuide/UsingWithRDS.MasterAccounts.html)
- [MySQL 8.4 Reference Manual, InnoDB INFORMATION_SCHEMA Metrics Table](https://dev.mysql.com/doc/refman/8.4/en/innodb-information-schema-metrics-table.html)
- [MySQL 8.4 Reference Manual, Server Status Variables](https://dev.mysql.com/doc/refman/8.4/en/server-status-variables.html)
- [MariaDB Documentation, InnoDB Status Variables (Innodb_deadlocks)](https://mariadb.com/docs/server/ha-and-performance/optimization-and-tuning/system-variables/innodb-status-variables)
