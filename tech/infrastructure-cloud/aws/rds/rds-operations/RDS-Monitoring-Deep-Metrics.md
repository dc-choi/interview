...
어떤 SQL, 호스트, 사용자, 대기 이벤트가 부하를 만드는지 보여준다. **장애가 난 뒤 켜면 과거 원인 분석이 어렵다.** Performance Insights 독립 콘솔은 2026-07-31 종료되므로, 핵심 서비스 DB는 Database Insights Standard 또는 Advanced mode와 retention을 미리 선택한다. Performance Insights API는 유지된다(상세: [[RDS-Monitoring#Database Insights와 Performance Insights 전환]]).
