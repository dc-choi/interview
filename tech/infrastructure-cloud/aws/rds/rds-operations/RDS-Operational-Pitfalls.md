...
RDS가 죽는 게 아니라 **커넥션이 말라서 앱이 죽는다.** `max_connections`는 인스턴스 메모리에 비례한다. 파라미터 그룹에 수식으로 박혀 있다.
...
작은 인스턴스일수록 천장이 낮고 실제 값은 대상 parameter group에서 확인해야 한다. NestJS 앱을 ECS로 가로 확장하면 **태스크 수 × 풀 사이즈**만큼 커넥션이 곱해진다. Prisma v6 이하의 기본 풀은 `물리 CPU × 2 + 1`이라 4코어 태스크 10개면 9 × 10 = 90개다. v7부터 관계형 데이터소스는 driver adapter가 기본이라 mariadb adapter의 `connectionLimit`과 pg adapter의 `max` 기본값인 10이 적용된다. 사용 중인 Prisma 메이저 버전과 드라이버 설정을 확인해 역산해야 한다. 여기에 마이그레이션, 배치 잡, 다른 서비스까지 더하면 금세 천장을 치고 `Too many connections`가 발생한다.

```
# Prisma v6 이하에서는 URL parameter로 풀을 조인다
DATABASE_URL="mysql://user:pw@host:3306/db?connection_limit=10&pool_timeout=20"
```

Prisma v7에서는 URL parameter가 아니라 driver adapter 옵션으로 설정한다. MySQL/MariaDB는 `new PrismaMariaDb({ ..., connectionLimit: 10 })`, PostgreSQL은 `new PrismaPg({ connectionString, max: 10 })`처럼 풀 상한을 지정한다.
...
- **Lambda 인스턴스가 늘면 각 인스턴스의 풀이 누적**된다. 동시성 상한과 pool size를 함께 계산하고, 급격한 확장이 예상되면 [[RDS-Connection-Credentials|RDS Proxy]] 같은 외부 풀링을 검토한다.
...
Multi-AZ DB instance failover는 보통 **60~120초** 걸린다. Aurora는 일반적으로 60초 안에 복구되고 종종 30초 미만이다. 이 과정에서 기존 연결은 다시 맺어야 하고 진행 중 작업은 실패하거나 롤백될 수 있으며, 엔드포인트의 DNS record가 새 primary를 가리키도록 바뀐다.
...
- 연결 재시도는 실제 failover 관측 시간에 맞춘 지수 백오프와 지터를 쓰고 전체 시간 예산을 둔다. 결과를 모르는 쓰기나 트랜잭션을 통째로 재실행하지 말고, 멱등성이 보장된 작업만 재시도한다.
...
복제는 비동기다. 게다가 MySQL 바이너리 로그 복제는 전통적으로 단일 스레드로 재생되므로(병렬 복제 미설정 시) 마스터 쓰기가 몰리면 복제본이 뒤처진다. CloudWatch 지표 이름은 엔진과 관계없이 `ReplicaLag`다. MySQL/MariaDB에서는 `SHOW REPLICA STATUS`의 `Seconds_Behind_Master`, PostgreSQL에서는 `pg_last_xact_replay_timestamp()` 기반 값을 보고하며, `ReplicaLag = -1`은 복제가 활성 상태가 아님을 뜻한다.
...
장기 실행 트랜잭션의 디스크 부풀림, cross-AZ 전송비, Aurora I/O 과금, utf8/utf8mb4, CA 인증서 만료, 암호화 전환 시 encrypted snapshot copy와 새 instance가 필요한 점, 로그 디스크 잠식, Read Replica 승격 뒤 새 replica 구성이 필요한 점, max_allowed_packet, IAM 토큰 유효시간, PostgreSQL XID wraparound는 빈도는 낮지만 처음 당하면 원인 찾기가 어렵다. → [[RDS-Operational-Pitfalls-Rare|RDS 운영 함정 (저빈도, 진단 어려운 것들)]]
...
- [Prisma ORM, Connection pool](https://www.prisma.io/docs/orm/prisma-client/setup-and-configuration/databases-connections/connection-pool)
- [Amazon RDS, Monitoring read replication](https://docs.aws.amazon.com/AmazonRDS/latest/UserGuide/USER_ReadRepl.Monitoring.html)
- [Amazon RDS, Modifying parameters in a DB parameter group](https://docs.aws.amazon.com/AmazonRDS/latest/UserGuide/USER_WorkingWithParamGroups.Modifying.html)
