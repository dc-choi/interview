...
- **롤백** — **소스를 바로 지우지 않는 것**이 중요하다. 검증될 때까지 살려둔다. 단 새 타깃에서 쓰기가 이미 시작됐으면 단순 롤백 시 그 사이 데이터를 잃는다. 진짜 안전하게 가려면 **역방향 복제(타깃 → 소스)**까지 거는데 난이도가 확 오른다. 보통은 "검증을 빨리 하고, 문제 시 짧은 윈도 안에 되돌린다"로 타협한다.
