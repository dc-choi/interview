...
**HTTP/2 종단**: ALB는 HTTPS 리스너에서 클라이언트와 HTTP/2를 자동 협상(ALPN)하고, 미지원 클라이언트에는 HTTP/1.1로 폴백한다. 단 **백엔드 대상과는 기본적으로 HTTP/1.1로 통신**하므로 EC2, 컨테이너 앱은 평소대로 HTTP/1.1만 처리하면 된다 — TLS 종단과 같은 엣지 종단 모델이다(CLB는 HTTP/2 미지원이라 ALB 마이그레이션 필요). 프로토콜 자체는 [[HTTP-2|HTTP/2]].
