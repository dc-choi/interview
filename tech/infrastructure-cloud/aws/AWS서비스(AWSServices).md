...
- [x] [[ECS|ECS, Fargate (Task Definition, Service Connect, awsvpc, Capacity Provider, IAM Role 3종)]]
  ...
  - [[ECS-SQS-Worker-Terraform|SQS 워커 오토스케일링 Terraform 폴더 인덱스 (공통 리소스 + Fargate/EC2 옵션)]]
    - [[ECS-SQS-Worker-Terraform-Common|공통 리소스 (큐/DLQ, 최소 권한 IAM, Container Insights, backlog-per-task Metric Math)]]
    - [[ECS-SQS-Worker-Terraform-Launch-Types|옵션 A Fargate, 옵션 B EC2 (Capacity Provider, Managed Scaling, ASG)]]
...
- [x] [[Elastic-Beanstalk|Elastic Beanstalk (애플리케이션 PaaS, 환경, 배포 정책, .ebextensions, platform hooks)]]
- [x] [[AWS-Lambda|AWS Lambda, 서버리스 FaaS (Cold Start, Provisioned/Reserved Concurrency, VPC Lambda, Destinations, Lambda@Edge, MicroVMs)]]
- [x] [[Step-Functions|Step Functions (Standard/Express, 상태 머신, 통합 패턴, Retry/Catch, 보상)]]
...
- [x] [[S3-File-Upload|S3 파일 업로드 (서버 경유 Stream, MultipartFile / 클라이언트 직접 전송 Multipart Upload, Presigned URL / 보안과 운영, CORS, 선택 가이드)]]
...
- [x] [[Cognito|Cognito (User Pool 인증, Identity Pool 임시 자격 증명, OAuth2/OIDC, Federated Identity, Lambda Trigger)]]
...
- [x] [[RDS-Aurora|RDS / Aurora 관리형 DB (Multi-AZ, Read Replica, Failover, 백업/PITR, RDS Proxy, Aurora Endpoint, Global DB)]]
  - [[RDS-Aurora-Endpoints|Aurora Endpoint 운영 (Custom Endpoint 워크로드 격리, Reader→Writer 폴백, Failover read-only + JDBC Wrapper)]]
  - [[RDS-Aurora-AutoScaling|Aurora Auto Scaling 운영 (Custom Metric 배치 제외, Target Tracking vs Step Scaling, Flapping, Cooldown, Cache Warming)]]
  ...
  - [[RDS-Monitoring|모니터링 (지표와 알람 기준, 로그와 Slack 알람 파이프라인, 외부 도구와 운영 함정)]]
