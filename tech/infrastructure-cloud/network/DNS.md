...
도메인 이름을 IP 주소로 변환하는 인터넷의 분산 디렉터리. 단순 변환기가 아니라 전 세계 수억 개의 도메인을 한 서버에 몰지 않고 루트 → TLD → 권한 네임서버로 이어지는 계층 구조로 나눠 관리하는 시스템이다. 사람은 `google.com`을 기억하고, DNS는 그 이름에 해당하는 `142.250.207.46` 같은 주소를 찾아 준다. AWS의 구현(호스팅 영역, 라우팅 정책, Alias)은 [[Route53]] 참고.
