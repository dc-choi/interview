...
- **Under-provisioning**: Memory는 OOMKill, CPU는 Throttling으로 직결된다.

`requests`는 단순 설정값이 아니라 **스케줄링 예약**이다. Pod가 실제로 얼마를 쓰든 requests에 선언된 만큼은 이미 점유된 것으로 처리된다.
...
CPU는 사용률만 봐서는 안 된다. 사용률이 낮아도 **Throttling 중일 수 있고**, 그 경우 requests를 내리면 상황이 악화된다. 사용률 + Throttling 비율을 항상 함께 해석해야 한다.
...
| 낮음 | 있음 | Limit이 너무 낮음 — Limit/Request 상향 |
| 높음 | 없음 | 적정 |
| 높음 | 있음 | 부족 — Request/Limit 상향 |
