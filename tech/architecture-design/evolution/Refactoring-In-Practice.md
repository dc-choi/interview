...
레거시가 무섭다는 감정의 실체는 코드가 아니라 **책임에 대한 두려움**이다 — 건드렸다가 망가지면 어떡하나. 이 두려움을 피해 레거시를 우회해도 결국 어디서든 다시 만난다. 레거시 없는 회사는 드물다.
...
두려움은 당연하고, 건드리기 시작하면 줄어든다. 남의 코드를 자신 있게 고칠 수 있을 때 얹혀가는 개발자에서 시스템을 주도하는 개발자로 올라선다.

## 죽은 코드는 지운다

- 주석으로 덮어둔 옛 코드와 사용되지 않는 클래스는 빌드에 영향이 없어도 **읽는 사람에게 비용**을 만든다 — 살아 있는 조건인지 죽은 로직인지 판단하려고 결국 다시 읽고 분석하게 된다.
- 지우지 못하는 이유는 작성에 들인 시간이 아깝다는 감각과 나중에 쓸지 모른다는 불안이지만, 커밋해 둔 상태는 대개 Git 히스토리에서 되살릴 수 있다. 레포지토리를 백업 창고가 아니라 깨끗한 전시관으로 다룬다.
- 실력은 추가가 아니라 삭제에서 드러난다 — 코드를 줄일수록 읽을 것이 줄고 버그가 설 자리도 줄어든다.

## 시간이 없다는 말 — 리팩토링은 이벤트가 아니다

- **별도 리팩토링 기간 요청은 대개 거절당한다.** 이미 잘 동작하는 코드에 일주일의 공수와 위험을 들일 이유가 회사에는 없다. 리팩토링은 날을 잡아 하는 이벤트가 아니라, 변수 이름을 바꾸고 긴 메서드를 쪼개는 몇 분 단위 습관의 누적이다 (정리 시점의 선택지는 [[Tidy-First|Tidy First]] 참조).
- **시간이 없다는 말의 본심도 위의 책임 두려움인 경우가 많다** — 고쳤다가 장애가 날까 봐. 리팩토링 속도를 결정하는 것은 여유 시간이 아니라 테스트의 존재다. 소형 단위 테스트 몇 개만 있어도 함수 내부를 다듬는 일은 몇 분이면 되고, 이것이 아래 2단계 안전망과 같은 축이다.
- **티켓을 마무리하기 직전 15분의 설거지** — 방금 쓴 코드를 다시 읽으며 변수명이 의도를 담는지, 한 함수가 과다한 역할을 맡지 않았는지 확인한다. 위 레거시 이름 변경과 달리 방금 쓴 내 코드는 맥락이 살아 있을 때가 가장 싸게 다듬는 시점이다. 독립 리팩토링 티켓은 잘 주어지지 않으므로, 업무 루틴 안에 스스로 포함시켜야 품질이 유지된다.
...
- [레거시 코드가 무서운 개발자에게 — Team Grit](https://teamgrit.co/article/346)
