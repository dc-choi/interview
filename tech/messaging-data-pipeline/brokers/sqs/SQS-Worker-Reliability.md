...
insert-first 패턴([[At-Least-Once|At-Least-Once]])처럼 처리 상태를 DB에 `PROCESSING`으로 남기는 설계에서, 워커가 죽으면 그 행이 `PROCESSING`으로 굳는다. 가장 나쁜 조합은 SQS 가시성이 만료돼 메시지가 다시 나오는데 DB가 `PROCESSING`이라 다른 워커가 처리 중으로 판단하고 매번 건너뛰는 경우다. 재시도는 도는데 아무 일도 일어나지 않고, 결국 `maxReceiveCount`를 채워 DLQ로 간다.
