...
| **Kinesis Data Firehose** | 캡쳐·변환·전송 (S3·Redshift·OpenSearch·Splunk) | 시험 출제 핵심 |
| **Kinesis Data Analytics** | SQL/Python/Scala로 스트림 분석 | 보조 |
| **Kinesis Video Streams** | 영상 스트리밍 수집·저장 | 출제 빈도 낮음 |

SAA에서 "Data" 3종(Streams·Firehose·Analytics) 위주로 출제된다.
...
- **Kinesis Data Firehose** (변환·전송 파이프라인으로 위임)
...
실시간 스트림을 **SQL·Python·Scala**로 처리·분석. Apache Flink 기반 (Managed Service for Apache Flink로 리브랜딩).
...
| 처리량 단위 | 무제한(Standard) / 300 TPS(FIFO) | 무제한(Standard) | **Shard 단위 1MB/s·1,000 rec/s** | Partition 단위 |
...
- Kinesis 4종 = **Data Streams · Data Firehose · Data Analytics · Video Streams** (Data 3종이 시험 핵심)
...
- Firehose는 **변환·전송 전용**, 데이터 보존 X, 대상은 **S3·Redshift·OpenSearch·Splunk**
- KCL(Consumer Library) / KPL(Producer Library)
- 실시간 분석은 Data Analytics(SQL/Python/Scala, Flink 기반)
