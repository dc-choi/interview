...
재직 중 직접 이 구성으로 IoT 재고관리(VMI) 서비스의 발주 자동화를 운영하면서, DLQ를 큐마다 같은 정책으로 두지 않고 실패의 성격에 맞춰 나눴다. 전제는 하나다. 재시도로 나아지는 실패만 재시도한다.

- **수주 처리 DLQ (보상이 필요한 실패)**: 실패 메시지를 감지하면 먼저 해당 발주의 현재 상태를 조회하고 롤백을 시도한다. 롤백이 성공하면 만들어진 발주 데이터를 삭제하고 오류로 종결, 실패하면 긴급 알림을 띄우고 수동 처리로 넘긴다. 앞 단계가 이미 커밋된 뒤라 단순 재시도가 아니라 보상 동작이 필요했다.
- **알림톡 DLQ (영구 오류와 일시 오류 분기)**: 잘못된 수신 번호처럼 몇 번을 보내도 결과가 같은 영구 오류는 재시도 없이 즉시 실패 처리한다. 네트워크 오류와 서버 부하는 점진 재시도로 회복을 기다린다.
