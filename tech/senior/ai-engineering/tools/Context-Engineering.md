...
> **제품 동작 vs 이 vault의 적용 규칙 분리**: 위 MEMORY.md 자동 주입은 Claude Code 제품이 제공하는 일반 동작 설명이지 모든 환경에 강제되는 원칙이 아니다. 이 vault는 auto memory 시스템을 쓰지 않기로 정했다 — 세션 간 유지가 필요한 정보는 MEMORY.md가 아니라 CLAUDE.md의 User Context 섹션에만 기록한다(git으로 여러 기기 동기화, 이력 추적). 즉 위 우선순위는 메커니즘을 이해하기 위한 참고이고, 이 레포의 실제 운용은 CLAUDE.md 중심이다.
...
코딩 작업은 모델의 디폴트 행동이 강해서, "해야 할 것"보다 **"하지 말 것"을 명시**하는 게 더 효율적이다. 65줄짜리 부정형 룰셋 하나로 대표 실패 패턴(가정 질주, 과잉 복잡화, 무관 코드 수정, 검증 없는 완료 선언)을 차단한다 — 200줄 예산 안에 들어가는 압축 룰셋의 예. 실패 패턴별 원칙, 판정 테스트, 배포 경로는 [[Agent-Coding-Guardrails|LLM 코딩 가드레일]]이 정본.
...
- [[Agent-Coding-Guardrails|LLM 코딩 가드레일 (부정형 룰셋의 실패 패턴별 원칙, 판정 테스트, 배포 경로)]]
