...
재직 중 직접, AI 코드 생성만 쓰던 팀에 하네스를 도입했다. 도입 전에는 Deprecated 도메인을 건드린 수정 사고, 컨텍스트 폭주, 세션 종료 시점의 품질 회귀가 반복됐다. 결과가 개인 프롬프트 실력에 좌우되는 상태였다.

- **Constrain** — 계층형 CLAUDE.md를 3단으로 분할하고, 사고가 났던 Deprecated 도메인 13종을 명문화해 원천 차단
- **Inform** — 도메인 규칙을 대상 경로에 두어 관련 작업에서만 로드
- **Verify** — Stop Hook에 자가 리뷰를 걸어 git diff를 확인한 뒤 버그 점검과 품질 리뷰를 수행(180초 타임아웃), PostToolUse Hook은 편집한 파일만 lint와 format
- **워크플로우 자동화** — 반복 작업을 Subagent 3종과 Skill 3종으로 분리

효과는 사람이 코드 생성이 아니라 설계 리뷰에 시간을 쓰게 된 것이다. 운영 배포는 2026년 1월부터 4월까지 126건으로, 4개월 만에 전년 연간 84건의 150%에 도달했다(월평균 7건에서 31.5건, 누적 환산 기준).

school-manage가 개인 프로젝트에서 하네스 구성 요소를 갖춘 사례라면, 이쪽은 여러 명이 같은 하네스를 공유할 때 무엇이 먼저 값을 하는지를 보여준다. 팀에서는 Constrain(금지 도메인 명문화)과 Verify(Stop Hook 자가 리뷰)가 먼저였다. 개인은 규칙을 어겨도 본인이 알아채지만, 팀은 어긴 사실을 아무도 모른 채 배포되기 때문이다.
