---
tags: [senior, ai, harness, architecture, index]
status: index
...
에이전트를 감싸는 제어 계층 — 하네스 5원칙, 레포와 조직 단위의 결정론적 강제, 프로덕션 에이전트 아키텍처. 상위: [[tools|AI 엔지니어링 실천 도구]].

- [[Harness-Engineering|하네스 엔지니어링]]: Constrain→Inform→Verify→Correct→HITL, 프롬프트→컨텍스트→하네스→루프 진화, 멀티 에이전트 오케스트레이션, school-manage 현장 적용
