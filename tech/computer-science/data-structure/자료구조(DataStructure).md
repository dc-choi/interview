...
자료구조란 데이터가 어떤 구조로 저장되고 어떤 상태를 가지고 있는지에 대한 정보를 가지고 있는 것이다. 가장 단순한 자료구조는 변수이고, 그 다음은 배열이다.
...
운영체제는 배열의 시작주소만 기억하고, 시작주소부터 연속적으로 메모리에 값을 할당한다.
배열은 읽기/쓰기(참조)의 성능이 좋지만, 삽입/삭제의 성능이 좋지 않다. 크기 예측이 힘들어 메모리 낭비가 심할 수 있다.

#### JS에서의 배열
상황에 따라서 연속적, 불연속적으로 메모리를 할당한다. 대부분의 상황에서 불연속적으로 메모리를 할당하며, 내부적으로 연결해서 사용자에게는 배열인 것처럼 보이게 한다.

V8이 단일 타입, hole 없는 배열을 연속 메모리로 최적화하는 규칙(elements kinds)과 Typed Array는 [[V8-Array-Internals|V8 배열 내부 구현]] 참고.
...
### [LinkedList](linked-list/LinkedList.mjs)란?
...
운영체제에서 FIFO 스케줄링 등에 사용된다.

버퍼, 마구 입력된 것을 처리하지 못하고 있는 상황, BFS에서 사용된다.

Queue의 가장 첫 원소를 front(deQueue 할 위치), 끝 원소를 rear(enQueue 할 위치)라고 부르고 Queue는 들어올 때 rear로 들어오지만, 나올 때는 front부터 빠지는 특성을 가짐.

가장 첫 원소와 끝 원소로만 접근 가능하며 데이터를 넣고 뺄 때 해당 값의 위치를 기억해야 함. (스택에서 스택 포인터와 같은 역할)

Queue를 연결리스트로 구현한 예제
```Java
public class Node<T> {
...
### Deque(덱)이란?
front와 rear 두 곳에서 전부 데이터를 입력하고 삭제할 수 있는 자료구조이다.
배열이나 연결 리스트 둘 다 덱을 구현할 수 있지만, 연결 리스트를 사용하는 것이 효율이 더 좋다.

### HashTable이란?
해시테이블은 프로그래밍 언어마다 다른 이름을 가지고 있다. (해시, 맵, 해시맵, 딕셔너리)
해시 함수로 테이블의 인덱스를 새로 만들기 때문에 해시테이블이라는 이름이 붙었다.
빠른 데이터 읽기, 삽입, 삭제가 가능하지만 공간의 효율성이 좋지 않다.
해시 함수를 통해 테이블에 데이터를 골고루 저장할 수 있는 것이 좋은 해시테이블이다.

개념 토대(직접 주소 테이블, 해시 함수, 적재율, 리사이즈)는 [[Hash-Table|Hash Table]], 해시 충돌 해결 전략(Chaining, Open Addressing, Java 8 트리 전환, HashDoS, Load Factor)은 [[Hash-Collision|Hash Collision]] 참고.

### Set이란?
데이터의 중복을 허용하지 않는 자료구조이다.
해시테이블을 사용해서 해시셋이라고도 불린다.
셋은 해시테이블의 value 값은 사용하지 않고 key만 사용해서 구현하며, key가 키이자 값으로 사용된다.
...
배열 구현(인덱스 수식), 버블 업/트릭클 다운, 힙 정렬 등 상세는 [[Heap|Heap]] 참고.
