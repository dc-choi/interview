...
미용의료 플랫폼 강남언니(운영사 힐링페이퍼)는 2026년 상담 내역 조회 API의 비정상 접근으로 약 21만 9천 명의 개인정보가 유출됐다. 이름과 연락처를 넘어 병원과 의사명, 시술 내역, 결제 정보까지 응답에 담겼다.

- **API 객체 수준 인가 부재**: 클라이언트가 지정한 대상의 상담 내역을 소유권 검증 없이 반환하는 것은 IDOR을 API로 옮긴 BOLA(Broken Object Level Authorization, OWASP API Security 1위)의 전형이다
- **과다 데이터 노출**: 화면에 필요한 범위를 넘어 민감 필드까지 그대로 응답에 실어 피해를 키웠다. 응답은 클라이언트 필요 최소로 좁히고, 민감 필드는 분리하고 최소 수집한다
- **사고 대응의 함정**: 1차로 문제 경로를 차단했으나 다음 날 같은 공격자가 다른 경로로 재차 침입을 시도했다. 한 엔드포인트만 막는 것으로는 부족하고, 같은 인가 결함 패턴을 공유하는 형제 엔드포인트 전체를 함께 점검해야 한다
...
- [[Access-Control-Models|접근 제어 모델 (RBAC, ABAC, PBAC)]]
- [[SSRF|SSRF]]
- [[Session|세션]]
- [[JWT|JWT]]
