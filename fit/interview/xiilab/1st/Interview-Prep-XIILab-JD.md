...
| React 사용 경험 | **중** | 사이드 프로젝트 출석부(school-manage)에서 React 19 + Vite + TypeScript 풀스택 개발, 운영. 2026-07-15 보고서상 모임 레코드 108개 |
...
| JD 스택 | 내 경험 | 갭 분석 |
|---------|--------|--------|
| React | React 19 (사이드 프로젝트 출석부 — 2026-07-15 보고서상 모임 레코드 108개) | 프로덕션 경험 있음 |
...
트라이포드랩에서 NestJS 기반 VMI 서비스의 핵심 기능을 설계하고 운영해왔습니다. 850대 IoT 환경에서 같은 품목의 동시 갱신을 DB Lock으로 해결하고, EventBridge와 SQS 기반 이벤트 아키텍처로 수기 발주 프로세스를 완전 자동화했습니다. 운영 중에는 복합 인덱스 최적화로 슬로우 쿼리를 99.3% 개선하고, 단일 서버 구조를 CloudFront+ECS 아키텍처로 전환해 안정적인 스케일링 환경을 마련했습니다.
...
- **내 연결점**: 트라이포드랩에서 제품의 0→1 구축부터 대형 고객사 PoC, 운영 최적화까지 전 과정을 경험. 특히 850대 IoT 운영 환경에서 같은 품목 5개 writer 경합의 정합성 검증, 이벤트 기반 자동화, 인프라 고도화를 주도
...
- 꼬리 대비: "React 경험 없는데 괜찮나?" → 사이드 프로젝트 출석부를 React 19 + tRPC로 직접 개발해 2026-07-15 기준 MAO 34곳, 누적 본당 85곳, 학생 3,669명, 출석 약 22,051건의 프로덕션 서비스를 운영. 커스텀 훅, React Query, 인증 흐름 등 실전 경험이 있고, AI 도구(Claude Code)를 활용해 부족한 부분은 빠르게 보완할 자신 있음
