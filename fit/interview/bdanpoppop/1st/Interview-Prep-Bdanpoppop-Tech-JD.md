...
> 관련: [[REST|REST]], [[API-Design|API 설계]]

- REST 원칙: 자원 중심 URI, HTTP 메서드 의미 일관성, 무상태성, HATEOAS는 실무에서는 거의 안 씀
- 멱등성: GET·PUT·DELETE 멱등, POST 비멱등. 결제·교환 같은 비멱등 작업은 **Idempotency-Key 헤더**로 멱등 보장
...
> 관련: [[Index|인덱스]], [[Execution-Plan|실행계획]], [[Database-Design|DB 설계]]

- 정규화 vs 역정규화 트레이드오프: 정규화는 무결성·저장공간, 역정규화는 조회 성능. 읽기 부하 큰 도메인(상품권 매물 검색)은 검색 전용 역정규화 테이블 검토
...
- **Write-Through**: DB·캐시 동시 갱신. 일관성 ↑, 쓰기 비용 ↑
...
> 관련: [[MQ-Kafka|MQ·Kafka]], [[Messaging-Patterns|메시징패턴]]
...
## 5. 대용량 트래픽 처리·튜닝 — 우대 (중 매칭)

> 관련: [[Scaling|확장성]], [[Load-Balancing|로드밸런싱]]

**트래픽이 증가했을 때 백엔드 단계별 대응**
1. **수직 확장**: 인스턴스 사양 ↑ — 단순하지만 한계·SPOF
...
7. **DB 샤딩**: 트래픽이 한 DB의 한계를 넘으면. 샤드 키 설계·재샤딩 비용이 큼

**팝팝 시나리오 — 신상품권 발매·이벤트 시 트래픽 스파이크**
...
> 관련: [[Docker|Docker]], [[Kubernetes|K8s]]
...
- 상품권 교환·소유권 이전을 체인에 기록 (감사 추적·신뢰성)
- STO 연계: 토큰화된 상품권을 토큰증권으로 발행·이전 (전자증권법상 분산원장)
- 사용자 지갑 연동 (Web3 지갑 주소 매핑·서명 검증)

**백엔드 엔지니어가 다루게 될 패턴**
- **RPC 호출**: Web3.js / Ethers.js (TypeScript에서는 Ethers.js가 표준). RPC 노드(Infura·Alchemy 또는 자체 노드) 호출
- **이벤트 리스닝**: 컨트랙트 이벤트 구독 — `eth_subscribe`(WebSocket) 또는 폴링·Webhook
...
- 외부 API 호출(체인 RPC) = 결제·발행사 API 호출 패턴
- 비동기 워커 + 재시도·DLQ = SQS 발주 워커 패턴
- 멱등성 키·status 머신 = 발주 처리 패턴
- **즉 도메인 지식만 학습하면 백엔드 인프라는 그대로 옮겨 쓸 수 있다**고 어필

**꼬리 질문 대비**
- "어떤 체인을 쓰나?" → 비단팝팝은 "자체 블록체인" 명시. 이더리움 호환 체인(EVM)일 가능성 높음. KDX 컨소시엄 인프라(분산원장)와 연동 추정. **면접에서 어떤 체인·노드 구성인지 역질문**
