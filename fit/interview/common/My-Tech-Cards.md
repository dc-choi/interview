...
- **카드 1**: IoT 수천 대 동시 정합성 — DB Lock 전략 (SELECT FOR UPDATE NO WAIT)
...
> 마스터 카드 본문이 얕게 느껴지면 아래 vault 파일을 직접 읽고 **마스터 본문에 콘텐츠 흡수** (위키링크가 아니라 수치, 비교, 꼬리 자체). 보강 워크플로우는 CLAUDE.md "기술 답변 보강 워크플로우" 참조.
