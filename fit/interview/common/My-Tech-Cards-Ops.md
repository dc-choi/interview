...
**꼬리 (핵심)**:
- **"오토스케일링 기준?"** → API 서버: CPU 70% or 요청 수 / 큐 워커: SQS `ApproximateNumberOfMessagesVisible` (큐 depth) 기반. 임계는 도메인과 부하 특성에 따라 다르게 잡습니다. API 서버는 보통 CPU 70% 근처를 기준으로 두지만, 갑작스러운 트래픽 증가에 여유를 더 두고 싶은 운영 초기에는 CPU 50%를 증설 신호로 보고 더 보수적으로 잡았습니다. 50%가 안정 운영 구간이라 그 위로 지속되면 증설하도록 두면 임계를 친 뒤 인스턴스가 뜨는 시간 동안의 여유가 확보됩니다. 처음엔 60%를 생각했는데 운영하며 더 보수적으로 내렸고, 부하 테스트로 실제 한계점을 본 뒤 다시 조정하는 식으로 갑니다.
