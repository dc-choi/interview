...
1. **경력 갭 정면 돌파** — JD는 5년+, 나는 **4년차**(약 1년 부족). "연차는 1년 모자라지만 다룬 문제의 *깊이와 범위*는 그 이상" — IoT 동시성 정합성, 100만 건, 850대 디바이스 환경의 인덱스 최적화, 아키텍처 전환, 관측성 구축, AI 파이프라인 재설계까지. 4개 회사 + 실서비스 사이드. **양이 아니라 밀도로 설득**.
2. **데이터 수집/처리 파이프라인 = 최강 무기** — JD 주요 업무 첫 줄이 "데이터 수집/처리 파이프라인 및 API 서버". 850대 IoT 디바이스 데이터 수집 파이프라인 경험이 *정확히* 직결. 면접 내내 이 축으로 답변을 모을 것.
