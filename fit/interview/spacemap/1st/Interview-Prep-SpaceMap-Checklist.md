...
1. **데이터 수집, 처리 파이프라인 전 구간 경험** — JD 주요 업무 첫 줄과 정확히 일치. 850대 IoT 디바이스 데이터를 수집(DB Lock 정합성)→분리(EventBridge/SQS)→저장(복합 인덱스)→노출(Read Replica)
2. **측정한 인덱스 개선** — 100만 건, 850대 환경에서 최신 상태 조회 쿼리를 15.4ms에서 0.1ms로 개선. 데이터가 늘면 실행계획, B-Tree 깊이, 캐시와 I/O를 다시 측정해 인덱스와 파티션을 조정
