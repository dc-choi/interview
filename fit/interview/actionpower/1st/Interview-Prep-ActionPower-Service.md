...
- Node.js Stream으로 chunk 단위 처리:
  ...
  - API 서버 → 오브젝트 스토리지: 수신하면서 동시에 GCS/S3에 `stream.pipeline()`으로 업로드하고 오류를 호출자에게 전파
...
- "Presigned URL의 보안은?" → 짧은 만료 시간과 제한된 객체 키를 사용하고, 업로드 방식이 지원하면 S3 POST policy 같은 조건으로 크기와 Content-Type을 제한. 서버는 완료 이벤트 뒤 실제 포맷, 크기와 악성 파일 여부를 다시 검증
...
- 실무 연결: 발주 자동화에서 채널별 DLQ 운영
  - 잘못된 번호와 주소 같은 영구 오류는 재시도하지 않고 수동 처리 대상으로 분류
  - 네트워크와 공급자 서버 오류 같은 일시 오류만 제한된 횟수로 재시도
  - 최종 실패 시 긴급알림+수동처리
...
- 다글로 현 단계(삼성물산, 대구시청 등 B2B 확장 초기)에는 논리적 격리가 현실적
  - 테넌트 범위의 Prisma Client extension 또는 Repository wrapper에서 `tenant_id` 조건을 일관되게 구성
  - PostgreSQL을 쓴다면 Row Level Security를 추가해 애플리케이션 조건 누락을 DB 경계에서도 차단
...
- 실제 경험 기반:
  ...
  2. 대시보드에서 영향 범위 파악 — `x-request-id`로 로그를 추적하고 method, route와 status로 집계한 메트릭에서 영향 API 확인. OpenTelemetry trace pipeline과 metric exemplar를 실제 운영했다는 근거는 없음
...
- 꼬리:
  ...
  - "장애를 미리 방지하는 방법?" → 현재는 정적 임계 경보로 조기 감지. 사용자 영향 SLI와 목표 기간을 합의하면 multi-window, multi-burn-rate SLO 경보로 개선. 카나리 배포, 로드 테스트와 코드 리뷰+테스트 게이트도 함께 사용

## 3-4. 컬처핏 질문 (1차는 직무 면접이지만 섞일 수 있음)

- [x] 장단점 — [[My-FIT-Answers#1. 장단점 (한 흐름으로 묶기)|FIT]] 실행력/성급함
- [x] 기획자 충돌 — [[My-FIT-Answers#2-A. 환불 정책 — 일정 vs 리스크 (메인 사례, 5/15 검증)|FIT]] 결제 환불 사례
- [x] 긴급 이슈 판단 — [[My-FIT-Answers-Extended#17. 긴급 이슈 동시 발생 — 판단 기준|FIT]] 비즈니스 임팩트 기준
...
- [ ] **추가**: "빠르게 변화하는 AI 기술을 어떻게 따라가나?"
  ...
  - **검증 프로세스**: 관심 기술 → 프로토타입(1~2일) → 기존 시스템 대비 정량 비교 → 도입 여부 판단. 트라이포드랩에서도 필요한 MSK Provisioned 구성의 고정비와 EventBridge+SQS의 사용량 과금을 당시 트래픽 조건으로 비교해 선택했으며, 검증되지 않은 총비용 절감률로 말하지 않음
