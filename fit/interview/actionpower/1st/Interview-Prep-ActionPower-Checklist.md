...
2. **850대 IoT 운영 환경의 동시 정합성** — SELECT FOR UPDATE NOWAIT + 재시도 로직으로 같은 품목 5개 writer 경합에서도 Lost Update 방지 (다글로 사용자 협업 정합성 문제와 연결)
...
4. **슬로우 쿼리 99.3% 개선(15.4ms→0.1ms) + API 90% 향상** — 100만 건, 850대 디바이스 환경에서 카디널리티 분석 기반 복합 인덱스로 최신 상태 조회 범위를 좁힘. 데이터가 늘면 실행계획, B-Tree 깊이, 캐시와 I/O를 다시 측정
