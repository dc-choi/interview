...
- 문제 1: **스케일링 불가** — 트래픽 급증 시 CPU/메모리가 한 서버에 집중, 850대 IoT + 웹 트래픽이 같은 인스턴스를 공유
...
- 효과:
  ...
  - 채널별 오류 분류와 DLQ로 실패 격리 — 잘못된 번호와 주소 같은 영구 오류는 재시도하지 않고, 일시 오류만 제한 재시도한 뒤 긴급 알림과 수동 처리
...
- 이벤트 흐름 전체의 관측성 개선 계획 — 발주 ID와 `x-request-id`를 이벤트 속성에 전파해 생성부터 알림 발송까지 로그를 연결. 실제 분산 trace pipeline 구축으로 말하지 않음
- 채널별 재시도 정책 세분화
