...
- 꼬리:
  ...
  - "이벤트 순서 보장이 필요하면?" → SQS FIFO 큐(MessageGroupId 기반 순서 보장. 일반 FIFO는 파티션당 비배치 300 API TPS, 최대 10개 배치 시 초당 3,000개 메시지다. 고처리량은 리전별 서비스 할당량과 MessageGroupId 분산을 확인한다) 또는 Kafka(파티션 내 순서 보장)
