...
[[Self-Analysis#백엔드 강점과 역할 적합 조건 (2026-09)|운영 백엔드 강점]]을 인프라 변경, 배포, 관측, 장애와 복구까지 책임지는 역량으로 확장한다. 목표는 백엔드 이해를 버리고 도구 목록을 늘리는 것이 아니라, 데이터 흐름과 애플리케이션을 이해하는 DevOps와 SRE 전환 기반을 만드는 것이다. 이 기반은 향후 플랫폼 엔지니어링으로 확장할 수 있지만, 이 로드맵만으로 플랫폼 역할 역량을 증명했다고 보지는 않는다. 2026-09-01 퇴사 결정은 이 트랙의 시작 조건이 아니다. [[Current-Goals-and-Roadmap|통합 로드맵]]의 선택 gate에서 남은 업무와 인수인계, 공개 가능한 경력 증거, 회복과 가용 시간, 이미 수락한 의무를 먼저 확인하고 DevOps를 유일한 트랙으로 고른 경우에만 시작한다. 학습 순서는 Terraform을 먼저 검증하고 다시 선택한 뒤 Kubernetes로 확장하는 것이다. 문서의 2027은 가장 이른 시작 목표다.

## 결정 요약

| 시기 | 실행 | 종료 조건 |
|---|---|---|
| 선택 전 | 남은 업무, 인수인계와 수락한 의무에 필요한 지식만 보충 | 실제 결과는 증거로 남기되 별도 로드맵 진도로 계산하지 않는다 |
...
검색이나 추천 로드맵의 완료는 시작 조건이 아니다. OpenSearch는 checkpoint를 보존한 채 보류 중이며, 그 보류가 DevOps를 자동 시작하는 근거가 되지 않는다. 시작 전에는 직접 닫은 운영 증거와 아직 독립적으로 소유하지 못한 인프라 빈칸을 구분하고, 통합 선택 gate의 네 조건을 먼저 확인한다. Terraform 최소 성공선을 닫으면 통합 gate로 돌아가며, 이 미래 투자를 계속하기로 다시 선택한 경우에만 Kubernetes를 연다.
...
DevOps를 시작한 뒤에는 다른 개인 학습 로드맵을 열지 않는다. 남은 업무, 인수인계나 수락한 의무에 필요한 긴급 학습은 예외이며 필요하면 DevOps를 보류한다. 핵심 2, 핵심 3과 핵심 4 뒤에는 통합 gate로 돌아가 다음 하나를 다시 고른다. EKS는 로컬 Kubernetes와 비용 통제를 검증한 뒤에만 별도로 판단한다.
...
- 남은 업무, 인수인계와 수락한 의무의 긴급 학습은 허용하되 로드맵 진도를 당겼다고 계산하지 않는다.
