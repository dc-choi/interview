...
category: "Growth - 학습"
aliases: ["2027 DevOps 실전 로드맵", "2027 데브옵스 학습 계획", "2027 백엔드 기반 DevOps 실전 로드맵"]
---

# 2027 백엔드 기반 DevOps 실전 로드맵

[[Self-Analysis#백엔드 강점과 역할 적합 조건 (2026-09)|운영 백엔드 강점]]을 인프라 변경, 배포, 관측, 장애와 복구까지 책임지는 역량으로 확장한다. 목표는 백엔드 이해를 버리고 도구 목록을 늘리는 것이 아니라, 데이터 흐름과 애플리케이션을 이해하는 DevOps와 SRE 전환 기반을 만드는 것이다. 이 기반은 향후 플랫폼 엔지니어링으로 확장할 수 있지만, 이 로드맵만으로 플랫폼 역할 역량을 증명했다고 보지는 않는다. [[Current-Goals-and-Roadmap|통합 로드맵]]의 선택 gate에서 현재 집중 자리, 실제 역할 수요와 가용 시간을 확인한 경우에만 시작한다. 학습 순서는 Terraform을 먼저 검증하고 다시 선택한 뒤 Kubernetes로 확장하는 것이다. 문서의 2027은 가장 이른 시작 목표다.

## 결정 요약

| 시기 | 실행 | 종료 조건 |
|---|---|---|
| 선택 전 | 백엔드 업무에서 필요한 운영 경계를 닫고, 즉시 필요한 지식만 보충 | 실제 결과는 전환 증거로 남기되 별도 로드맵 진도로 계산하지 않는다 |
...
검색이나 추천 로드맵의 완료는 시작 조건이 아니다. 다만 현재 검색 집중 자리가 실제 역할 수요로 점유 중이면 DevOps를 열지 않는다. 시작 전에는 직접 닫은 운영 증거와 아직 독립적으로 소유하지 못한 인프라 빈칸을 구분하고, 더 급한 실제 역할 기술과 회복을 먼저 비교한다. Terraform 최소 성공선을 닫으면 통합 gate로 돌아가며, 이 미래 투자를 계속하기로 다시 선택한 경우에만 Kubernetes를 연다.
...
- [ ] 준비 시작: 현재 집중 자리와 실제 역할 수요, 다른 학습 후보 및 생활 부담을 비교하고, 직접 닫은 운영 증거와 다음에 채울 인프라 빈칸 하나를 구분해 이 미래 투자를 선택했다.
...
DevOps를 시작한 뒤에는 다른 개인 학습 로드맵을 열지 않는다. 실제 역할 수요의 기술 학습은 예외이며 필요하면 DevOps를 보류한다. 핵심 2, 핵심 3과 핵심 4 뒤에는 통합 gate로 돌아가 다음 하나를 다시 고른다. EKS는 로컬 Kubernetes와 비용 통제를 검증한 뒤에만 별도로 판단한다.
...
- [ ] 미래 투자 선택 근거, 확인한 실제 역할 수요와 더 급한 응용 기술 빈칸, 직접 닫은 운영 증거와 채울 인프라 빈칸, 준비 구간 ID와 시작일을 기록했다.
...
- 한 주에는 새 개념 하나와 문제, 변경, 관측 및 복구를 연결한 검증 가능한 실습 하나만 다룬다.
- 읽기보다 구축, 실패 주입, 복구와 기록에 더 많은 시간을 쓴다.
- 업무 긴급 학습은 허용하되 로드맵 진도를 당겼다고 계산하지 않는다.
- 자격증은 핵심 4 종료 시점에 업무 필요, 공식 시험 가이드와 닫힌 실습 gate를 보고 판단한다. 열린 gate보다 시험 일정을 먼저 두지 않는다.

## 관련 문서

- [[2026-H2-Math-Roadmap|2026 하반기 수학 기초 로드맵]], [[2027-Search-Recommendation-Roadmap|검색 엔진 우선, 추천 시스템 전환 로드맵]]
- [[Self-Analysis#백엔드 강점과 역할 적합 조건 (2026-09)|백엔드 강점과 역할 적합 조건]], [[Engineer-Work-Loop|엔지니어 업무 성장 루프]], [[Performance-Evidence-Playbook|성과 증거 플레이북]]
