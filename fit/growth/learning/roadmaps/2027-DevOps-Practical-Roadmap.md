...
백엔드 서비스를 배포하고 관측하며 장애와 복구까지 책임지는 미래 역할 역량을 보강한다. [[Current-Goals-and-Roadmap|통합 로드맵]]의 선택 gate에서 현재 집중 자리, 실제 역할 수요와 가용 시간을 확인한 경우에만 시작한다. 학습 순서는 Terraform을 먼저 검증하고 다시 선택한 뒤 Kubernetes로 확장하는 것이다. 문서의 2027은 가장 이른 시작 목표다.

## 결정 요약

| 시기 | 실행 | 종료 조건 |
|---|---|---|
| 선택 전 | 실제 역할에 즉시 필요한 운영 지식만 보충 | 별도 진도표나 로드맵 진도로 계산하지 않는다 |
...
검색이나 추천 로드맵의 완료는 시작 조건이 아니다. 다만 현재 검색 집중 자리가 실제 역할 수요로 점유 중이면 DevOps를 열지 않는다. DevOps는 더 급한 실제 역할 기술과 회복을 먼저 비교해 선택한다. Terraform 최소 성공선을 닫으면 통합 gate로 돌아가며, 이 미래 투자를 계속하기로 다시 선택한 경우에만 Kubernetes를 연다.
...
- [ ] 준비 시작: 현재 집중 자리와 실제 역할 수요, 다른 학습 후보 및 생활 부담을 비교하고 이 미래 투자를 선택했다.
...
- [ ] 미래 투자 선택 근거, 확인한 실제 역할 수요와 더 급한 응용 기술 빈칸, 준비 구간 ID와 시작일을 기록했다.
