...
검색은 2026-08-10 당시 현재 역할과의 연결성을 근거로 개인적으로 선택했던 단일 집중 학습이다. 회사 지시나 평가 기준은 아니었다. 2026-09-01 퇴사 결정으로 [[OpenSearch|OpenSearch 학습 지도]]의 현재 checkpoint에서 보류했으며, 취소하거나 독립 fixture와 전체 진도를 자동 시작하지 않는다. 남은 업무와 인수인계, 공개 가능한 경력 증거, 회복과 가용 시간, 이미 수락한 의무를 먼저 확인한 통합 선택 gate에서 검색을 유일한 트랙으로 다시 고른 경우에만 재개한다. 검색은 추천의 수학적 선행조건이 아니며 문서의 2027은 정규 트랙의 가장 이른 시작 목표다.

## 시작 조건과 운영 원칙

- [[OpenSearch|OpenSearch 학습 지도]]의 checkpoint를 보존한다. 통합 선택 gate 전에 작은 gate, 독립 fixture와 전체 진도를 자동으로 재개하지 않는다.
- 독립 fixture를 쓰는 전체 정규 트랙은 통합 선택 gate에서 검색을 하나의 트랙으로 고르고, 공개 가능한 독립 fixture와 검토 조건이 있을 때만 시작한다.
- 이 로드맵을 진행하는 동안 다른 체계적 개인 학습 트랙을 병행하지 않는다.
- 검색 1차 성공선을 통과하고 통합 선택 gate에서 계속하기로 한 경우에만 추천 baseline으로 전환한다.
- 과거 또는 이후의 실제 역할 산출물이 해당 gate의 검증 조건을 충족하면, 민감 내용을 제외한 검증 근거를 남기고 별도 fixture를 다시 만들지 않는다. 기술을 사용했다는 사실만으로는 통과로 계산하지 않는다.
- DevOps는 이 로드맵의 다음 단계가 아니라 통합 선택 gate에서 따로 비교하는 미래 투자다.

## 검색을 먼저 하는 이유

1. 2026-08-10 당시 맡은 역할에서 검색 엔진의 mapping, analyzer, query와 품질 평가를 이해할 필요가 있었다.
...
flowchart LR
  ...
  B --> G{통합 선택 gate}
  G -->|추천 계속 선택| C[추천 모델링 baseline]
  G -->|다른 우선순위| X[다른 한 트랙 또는 보류]
  C --> I{통합 선택 gate}
  I -->|Taxonomy 계속 선택| D[Taxonomy 후보와 서빙]
  I -->|다른 우선순위| X
  D --> H{통합 선택 gate}
  H -->|검색과 추천 계속 선택| F[남은 discovery 단계]
  H -->|다른 우선순위| X
```
...
검색 1차 성공선을 통과하면 [[Current-Goals-and-Roadmap|통합 로드맵]]으로 돌아가 남은 업무와 인수인계, 공개 가능한 경력 증거, 회복과 가용 시간, 수락한 의무를 다시 확인한다. 검색 계속을 하나의 트랙으로 선택하고 공개 가능한 추천 fixture와 검토 조건이 있으면 C단계로 계속한다. 그렇지 않으면 이 모듈을 닫거나 보류한다.
...
위 최소 분모는 학습 보고서에서 지나치게 작은 구간을 구분하기 위한 탐색적 기준이지 통계적 검정력이나 성능 개선을 보장하는 합격선이 아니다. 성능 개선을 주장하기 전에는 비교할 baseline, 목표 또는 최소 탐지 변화, 대상 집단, 불확실성 표현과 관찰 기간을 별도로 정한다.
...
추천 baseline 성공선을 닫으면 통합 선택 gate로 돌아간다. Taxonomy를 계속할 명확한 목적, 공개 가능한 fixture와 검토 조건이 있을 때만 D단계를 고르고, 그렇지 않으면 이 모듈을 닫거나 보류한다.

## D. Taxonomy 후보와 logging/serving

이 단계는 MovieLens 지표를 그대로 이어 붙이지 않는다. 검색에서 사용한 공개 fixture로 돌아가고, A단계의 ID/event schema와 version 계약만 재사용한다. 승인되지 않은 내부 catalog나 운영 데이터를 사용하지 않는다.
...
이 gate를 통과하면 [[Current-Goals-and-Roadmap|통합 로드맵]]의 선택 gate로 돌아간다. 검색과 추천 계속을 하나의 트랙으로 다시 선택하면 Personalized Search, page 조립, OPE, 온라인 실험과 전체 장애 훈련 중 필요한 단계를 [[Search-Recommendation-Discovery-Learning-Path|전체 학습 경로]]에서 고른다. 그렇지 않으면 다른 한 트랙을 고르거나 새 트랙을 열지 않는다. OPE, Two-Tower, deep ranking, Transformer, GNN과 강화학습은 통합 선택 gate에서 독립 재현 목표를 확인한 때만 앞당기고, 다른 정규 트랙과 병행하지 않는다.
