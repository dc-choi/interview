...
검색은 2026-08-10부터 현재 역할과의 연결성을 근거로 개인적으로 선택해 진행 중인 단일 집중 학습이다. 회사 지시나 평가 기준을 뜻하지 않는다. 현재 검색의 작은 gate는 [[OpenSearch|OpenSearch 학습 지도]]에서 먼저 진행하며 수학 2단계 gate나 입사 첫 90일 회고를 기다리지 않는다. 이 문서의 독립 fixture와 전체 진도는 수학 2단계 gate와 입사 첫 90일 회고를 마친 뒤에도 검색 심화 필요가 이어질 때에만 다음 정규 개인 트랙으로 재검토한다. 검색은 추천의 수학적 선행조건이 아니며 문서의 2027은 정규 트랙의 가장 이른 시작 목표다.

## 시작 조건과 운영 원칙

- 현재 검색 학습에 필요한 작은 gate는 [[OpenSearch|OpenSearch 학습 지도]]에서 먼저 처리한다. 이 문서의 독립 fixture와 전체 진도는 자동으로 시작하지 않는다.
- 독립 fixture를 쓰는 전체 정규 트랙은 [[2026-H2-Math-Roadmap|수학 로드맵]]의 2단계 gate와 입사 첫 90일 회고 뒤에도 실제 역할 수요, 공개 가능한 독립 fixture와 검토 조건이 있을 때 선택한다.
- 이 로드맵을 진행하는 동안 다른 체계적 개인 학습 트랙을 병행하지 않는다.
- 검색 1차 성공선을 통과하고 통합 선택 gate에서 계속하기로 한 경우에만 추천 baseline으로 전환한다.
- 실제 역할 산출물이 해당 gate의 검증 조건을 충족하면, 민감 내용을 제외한 검증 근거를 남기고 별도 fixture를 다시 만들지 않는다. 기술을 사용했다는 사실만으로는 통과로 계산하지 않는다.
- DevOps는 이 로드맵의 다음 단계가 아니라 실제 역할 수요의 기술 빈칸이 없을 때 선택하는 별도의 미래 투자다.

## 검색을 먼저 하는 이유

1. 현재 역할에서 검색 엔진의 mapping, analyzer, query와 품질 평가를 먼저 이해할 필요가 있다.
...
flowchart LR
  ...
  B --> G{통합 선택 gate}
  G -->|실제 추천 역할 수요| C[추천 모델링 baseline]
  G -->|다른 우선순위| X[실제 역할 기술, 미래 투자, 범용 학습 또는 보류]
  C --> I{통합 선택 gate}
  I -->|실제 Taxonomy 역할 수요| D[Taxonomy 후보와 서빙]
  I -->|다른 우선순위| X
  D --> H{통합 선택 gate}
  H -->|실제 검색과 추천 역할 수요| F[남은 discovery 단계]
  H -->|다른 우선순위| X
```
...
검색 1차 성공선을 통과하면 [[Current-Goals-and-Roadmap|통합 로드맵]]으로 돌아가 실제 추천 역할 수요와 공개 가능한 추천 fixture 및 검토 조건이 있는지 다시 확인한다. 근거가 있으면 C단계로 계속하고, 실제 역할의 다른 기술 빈칸이 더 크거나 가용 시간이 부족하면 이 모듈을 닫거나 보류한다.
...
추천 baseline 성공선을 닫으면 통합 선택 gate로 돌아간다. 실제 Taxonomy 역할 수요와 공개 가능한 taxonomy fixture 및 검토 조건이 있을 때만 D단계를 고르고, 그렇지 않으면 이 모듈을 닫거나 보류한다.
...
이 gate를 통과하면 [[Current-Goals-and-Roadmap|통합 로드맵]]의 선택 gate로 돌아간다. 실제 검색과 추천 역할 수요가 이어지면 Personalized Search, page 조립, OPE, 온라인 실험과 전체 장애 훈련 중 필요한 단계를 [[Search-Recommendation-Discovery-Learning-Path|전체 학습 경로]]에서 고른다. 그렇지 않으면 다른 실제 역할 기술을 먼저 비교하고, 긴급한 빈칸이 없을 때만 DevOps 같은 미래 투자를 선택한다. OPE, Two-Tower, deep ranking, Transformer, GNN과 강화학습은 실제 역할 수요 또는 독립 재현 목표가 있을 때만 앞당기거나 병행한다.
