...
- 한 번에 초안 하나만 활성화한다.
- 선택한 개인 정규 트랙이나 공개 재현 실험과 연결된 글만 꺼낸다. 나머지는 기한 없는 백로그로 둔다.
...
검색, GraphQL, Redis와 Git은 공개 예제와 로컬 실습으로 검증할 수 있는 공통 기반이다. 공개 가능한 재현 실험에서 구체적인 문제, 선택과 결과가 생긴 글 하나만 활성화한다.

| 아웃풋 | 활성화 조건 | 상태 |
|---|---|---|
| 검색 | 공개 데이터로 문제를 재현하고 품질, 지연 시간 또는 운영 판단을 검증할 수 있음 | [ ] |
| GraphQL | schema, resolver, federation 또는 운영 판단 하나를 공개 가능한 예제로 재현할 수 있음 | [ ] |
