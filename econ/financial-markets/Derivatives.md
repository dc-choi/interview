---
tags: [econ, finance]
status: done
category: "Economics"
aliases: ["파생상품", "Derivatives"]
verified_at: 2026-07-21
---

# 파생상품

> 한 줄 요약: 파생상품은 다른 자산의 가격에서 가치가 파생되는 계약이다. 위험을 넘기는 보험으로도, 위험을 키우는 도박으로도 쓰인다. 차이는 레버리지를 어떻게 다루느냐다.

## 1. 파생상품이란 무엇인가

파생상품은 **기초자산, 금리, 지수 같은 기준 변수에서 가치가 파생되는 계약**이다. 옵션 프리미엄처럼 파생상품 계약 자체도 현재 시장가치를 가진다. 미래 현금흐름이나 가격 위험을 이전, 관리하거나 시장 전망을 표현하는 데 사용한다.

## 2. 세 가지 기본형

- **선물(futures)**: 미래의 정해진 시점에 정해진 가격으로 사고팔기로 하는 약속. 농부가 수확 전에 곡물 가격을 미리 고정하는 것이 원형이다. 양쪽 다 이행 의무가 있다.
- **옵션(option)**: 정해진 가격에 살(콜) 또는 팔(풋) **권리**를 사는 것. 의무가 아니라 권리라, 불리하면 행사하지 않으면 된다. 대신 그 권리의 값(프리미엄)을 미리 낸다.
- **스왑(swap)**: 두 당사자가 미래의 현금흐름을 맞교환하는 계약. 변동금리 이자를 고정금리로 바꾸는 금리스왑이 대표적이다.

## 3. 무엇에 쓰나 — 헤지, 투기, 차익거래

같은 도구가 목적에 따라 정반대 성격을 띤다.

- **헤지(위험 관리)**: 보유한 위험을 일부 또는 전부 상쇄하려는 포지션이다. 수출 기업이 예상 외화 수입을 선물환으로 고정하면 환율 변동 노출을 줄일 수 있지만 금액, 시점, 기준자산이 어긋나는 basis risk와 거래상대방, 유동성 위험은 남을 수 있다.
- **투기**: 가격 방향에 베팅해 수익을 노린다. 위험을 줄이는 게 아니라 새로 진다.
- **차익거래**: 이론적으로 같은 현금흐름의 가격 차이를 상쇄 포지션으로 포착한다. 실제 거래에는 실행, 자금조달, 거래상대방과 모델 위험이 있어 완전한 무위험 이익이 아닐 수 있다.

파생상품 자체가 선도 악도 아니다. 헤지에 쓰면 위험을 줄이고, 투기에 쓰면 위험을 키운다.

## 4. 레버리지 — 양날의 검

파생상품의 위력이자 위험은 **레버리지**다. 적은 증거금으로 큰 규모의 포지션을 잡을 수 있어, 가격이 조금만 유리하게 움직여도 수익률이 크게 뛴다. 반대로 움직일 때도 마찬가지다. 작은 역행에도 증거금을 넘는 손실이 날 수 있다. 같은 계약이 누군가에겐 보험이고 누군가에겐 파산의 지름길인 이유가 이 레버리지다.

## 5. 시스템 리스크

파생상품은 얽히고설키면 한 곳의 부실이 사슬을 타고 전체로 번질 수 있다. 2008년 금융위기에서 주택저당증권(MBS)과 그 부도를 보장하던 신용부도스왑(CDS)이 연쇄적으로 무너지며 위기를 키운 것이 대표 사례다. 개별로는 위험을 분산하는 도구가, 전체로는 위험을 한데 묶어 시스템 리스크를 만들 수 있다는 역설이다 → [[Business-Cycle|경기순환]].

## 6. 핵심 개념

- **선물, 옵션, 스왑**: 세 가지 기본형
- **헤지 vs 투기**: 위험을 줄이느냐 새로 지느냐
- **레버리지**: 수익과 손실을 함께 증폭
- **시스템 리스크**: 연결이 만드는 전염

## 7. 흔한 오해

- **파생상품은 도박이다** → 본래는 위험을 넘기는 보험이다. 도박이 되는 건 투기와 과도한 레버리지를 얹을 때다.
- **옵션은 사면 무조건 손해 볼 수 있다** → 옵션 매수자의 최대 손실은 낸 프리미엄으로 한정된다. 이론상 무제한 손실은 기초자산 가격 상한이 없는 **무담보 short call**에 해당한다. short put의 손실은 기초자산 가격이 0이 될 때까지로 제한되고, covered option이나 spread는 보유 자산과 다른 leg 때문에 손익 범위가 달라진다.
- **헤지하면 모든 불확실성이 사라지고 상승 여력도 포기한다** → 헤지는 특정 위험 노출을 줄이는 것이며 basis, 수량과 시점 불일치 때문에 잔여 위험이 남을 수 있다. 선도나 선물로 가격을 고정하면 유리한 움직임도 포기하지만, 보호적 옵션은 프리미엄을 내고 유리한 방향의 일부 상승 여력을 유지할 수 있다.

## 출처

- [CFTC Glossary, basis risk](https://www.cftc.gov/LearnAndProtect/AdvisoriesAndArticles/CFTCGlossary/index.htm)
- [CME Group, protective put example](https://www.cmegroup.com/education/articles-and-reports/trading-micro-e-mini-options)

## 관련 문서

- [[Risk-and-Return|위험과 수익]] — 파생상품이 옮기고 키우는 대상
- [[Financial-System-Overview|금융시스템 개관]] — 위험 이전이라는 시장 기능
- [[금융시장(Financial Markets)]] — 지도
