...
- 이 저장소는 개발 지식을 기반으로 대표에게 필요한 지식 전반을 축적하고 연결하는 개인 지식 저장소다. 개발, 제품과 사업, 브랜딩과 마케팅, 영업, 운영, 경제와 재무 등 실제 판단에 필요한 영역을 포괄하며 개발자 커리어나 학습 계획으로 목적을 한정하지 않는다. 기존 개발 지식과 로드맵도 사용자의 기반으로 유지한다.
- 지식 축적과 학습 로드맵을 함께 유지한다. 지식 문서는 판단에 필요한 근거를 쌓고, 학습 로드맵은 사용자가 그 지식을 이해하고 직접 판단할 수 있도록 학습 목적, 순서와 이해 확인 방법을 연결한다. 개발과 대표 역할에 필요한 지식 모두 이 대상이다. 문서 보유와 실제 숙련, 로드맵에 남긴 학습 범위와 현재 실행 중인 진도는 구분한다.
- 개인 지식과 온톨로지 구축 산출물은 이 저장소에서 관리한다. `kino-doc`은 회사에서 진행한 업무의 기록이며 개인 지식 작업의 기본 작성 위치나 모든 프로젝트에 적용할 규칙으로 취급하지 않는다.
...
- 기술 선택, 설계 변경과 신뢰성 검토에서는 사용자가 관련 지식을 지정하지 않아도 이 Vault의 원문과 현재 코드를 확인해 필요한 개선안을 제안한다. 커리어 판단에는 `fit/`, 비즈니스와 제품 판단에는 `biz/`, 경제와 투자 판단에는 `econ/`의 관련 원문도 필요할 때 조회한다. 학습 지식, 프로젝트 결정, 사용자 규칙, 경험 기록, 온톨로지의 설계와 운영 기록, AI 제안을 구분하고 프로젝트 결정은 명시된 범위에만 적용한다. 구축 코드와 문서는 루트 `ontology/`에서 관리하며 첫 진입점은 `ontology/Development-Ontology.md`다.
- 프로젝트 저장소는 현재 사업과 업무의 맥락을, 이 Vault는 판단에 사용할 지식을 관리한다. AI는 두 원문을 연결해 적용 이유, 대안, 판단을 바꿀 조건과 필요한 학습을 설명한다. 판단이나 학습 요청에서 맥락 요약과 분야 나열만으로 답변을 끝내지 않는다. 자료 조회나 요약만 요청한 경우에는 그 범위를 따른다.
- 프로젝트 조사에서 나온 로컬 경로, 체크아웃 상태와 일시적인 학습 추천을 별도 지식 지도나 현황 목록으로 상시 보관하지 않는다. 재사용할 개념, 적용 조건과 교훈은 기존 지식 원문에 보완하고, 조사 보고서나 학습 계획 자체는 사용자가 그 기록을 요청한 경우에만 남긴다.
- 학습 필요는 현재 결정과 사용자의 이해도에서 도출한다. 문서 보유를 숙련으로 간주하거나 확인한 사업을 초기 구상 단계로 되돌리지 않는다. 제안한 학습 순서와 실제 채택은 구분하며, 구체적인 연결과 학습 절차는 `development-context` 스킬과 `ontology/guides/Ontology-Context-to-Judgment.md`를 따른다.

## 작업 중 불확실성 확인

- 소스나 공식 자료로 확인할 수 있는 사실은 먼저 확인한다. 기존 요청과 대화로 의도가 정해졌으면 다시 묻지 않는다.
- 확인 뒤에도 결과의 방향을 바꾸는 의도나 범위가 불명확하면 필요한 질문만 하고, 답이 필요한 작업은 기다리되 독립적으로 진행할 수 있는 일은 계속한다. 주자료의 선택, 학습 순서의 확정, 기존 내용의 삭제나 보존이 이에 해당한다.
- 이미 승인된 범위의 가역적인 편집과 형식 선택은 문맥과 기존 관례에 따라 진행한다. 미확인 사실은 추정해 채우지 않고 미확인으로 표시한다.

## 검증 에이전트 운영

2026-09-22 사용자 요청으로 정한 기본 방식이다. 검증 에이전트를 사용할 때는 다음을 항상 적용한다.

- 사용 가능한 최상위 검증 모델과 그 모델이 지원하는 최대 추론 노력을 명시한다. 현재 기준은 Codex에서 `gpt-6-astra`, `ultra`이고, Claude에서는 사용자 전역 `~/.claude/CLAUDE.md`(import한 파일 포함)에 기준이 있으면 그 모델과 최대 노력을 따른다. 모델과 지원 범위가 바뀌면 다시 확인한다. 일반 구현 작업의 추론 설정까지 일괄 변경하는 규칙은 아니다.
- Fast 또는 priority 처리를 지원하면 함께 요청한다. 설정 요청과 실제 적용은 구분하고, 도구가 선택이나 확인 수단을 제공하지 않으면 적용됐다고 단정하지 않는다.
- 넓은 검증은 독립적으로 확인할 수 있는 도메인별 에이전트로 나눈다. 지적 사항과 수정 결과는 작성자와 별개의 검증 에이전트가 반증 관점에서 다시 확인한다.
- 검증 에이전트에는 결론을 그대로 넘겨 확증하게 하지 말고 대상 파일, 주장, 재현 조건과 원문 근거를 제공한다. 현재 코드, 호출부, 테스트, 설정과 공식 1차 출처를 대조한다.
- 확정 오류, 조건부 문제, 설명 보완과 미검증 사항을 구분한다. 테스트 통과나 에이전트 간 합의만으로 전체 내용이 맞다고 결론 내리지 않고, 재현 결과와 확인하지 못한 범위를 함께 보고한다.
...
- **Why**: 글로벌 git identity가 없는 상태로 커밋하면 `<user>@<hostname>` 같은 자동 추정 값이 공개 레포 히스토리에 박힌다.
...
- 일반 마크다운은 대략 190~200줄에서 분할을 검토한다. 줄 수는 검토 기준이며, 주제의 독립성과 가독성으로 결정한다. 한 흐름으로 읽는 편이 명확하면 유지하고, 작은 내용 추가만으로 구조 변경을 강제하지 않는다.
- 전체 분할이 필요하면 **Parent-as-TOC 패턴**을 기본으로 한다:
...
- 하나의 leaf 폴더(예: `tech/os-runtime/nodejs`)에 같은 주제 접두사를 공유하는 파일이 **3개 이상** 모이면 하위 폴더로 묶을지 검토한다. 독립된 주제 묶음이고 탐색이 쉬워질 때 분리하며, 파일 수만으로 이동을 강제하지 않는다.
