...
- **현업 예외**: GraphQL, gRPC, Kafka, Redis, Terraform, Kubernetes처럼 현업에 즉시 필요한 지식은 문제 해결에 필요한 범위만 학습할 수 있다. 현재 주축과 별개인 커리큘럼, 진도표나 자격증 준비로 확장하지 않으며 로드맵 단계 통과로 계산하지 않는다.
...
- **현재 실행 순서**: [[2026-H2-Math-Roadmap|수학 완전 기초]] → [[2027-Search-Recommendation-Roadmap|검색 엔진 baseline과 품질 평가]] → 같은 문서의 추천 baseline과 taxonomy 실습 → [[2027-DevOps-Practical-Roadmap|DevOps 실전]] 순서로 진행한다. 검색과 추천 디스커버리의 남은 고급 단계는 DevOps 핵심 4 통과 뒤에 재개하거나 현업 필요가 생긴 범위만 다룬다. 코딩 테스트는 제외한다.
- **gate 우선 전환**: 현재 트랙의 명시된 gate를 닫기 전에는 다음 개인 학습 로드맵을 열지 않는다. 문서의 연도는 가장 이른 시작 목표이며 마감일이 아니다. 선행 gate가 늦어지면 시간을 압축하거나 병행하지 않고 뒤 로드맵 전체를 순서대로 미룬다.
