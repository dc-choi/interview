...
- **가운뎃점(·, 미들돗) 사용 금지.** 항목을 나열할 땐 쉼표(,)나 자연스러운 연결어("~와", "~하고")로 잇는다. 면접 답변이나 폼 제출문 같은 **외부 제출 텍스트에서 특히 엄수**.
- 적용 범위: 새로 생성하는 모든 텍스트 (채팅 응답, 신규 문서, 폼 답변). 기존 vault 문서를 일괄 치환하진 않되, 편집으로 손대는 부분은 가운뎃점을 없앤다.
