...
- **예외**: 공개 출판물과 공개 발표의 저자, 발표자 표기(`## 출처`의 바이라인, 세미나와 밋업 기록의 발표자명, 강연자명 등)는 인용 귀속이라 유지할 수 있다.
...
- 분할 방식은 **Parent-as-TOC 패턴**을 기본으로 한다:
  ...
  3. 외부 위키 링크 `[[원본]]`은 그대로 유효하다. 대상 카테고리 인덱스 반영 여부는 해당 도메인 규칙을 따른다
...
- 10개 초과로 재분할할 때는 공유 접두사가 없어도 주제 단위로 하위 폴더를 만들 수 있다. 모든 분할에서 폴더 인덱스 파일명이 vault의 다른 파일과 동명(대소문자 무시)이 되거나 일반명사라 충돌 위험이 크면 `<폴더명>.md` 대신 대표 접두사형(예: `OpenSearch-Operations.md`, `JS-Prototype.md`)을 쓴다.
