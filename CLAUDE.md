...
- **아웃풋 우선**: 블로그, 면접 답변, 주일학교 플랫폼 개발에 시간 투자. 강의(input) vs 블로그(output) 충돌 시 output 우선. 단, 수학 완전 기초 단계는 개념 입력과 손계산 반복 자체가 필수 과정이므로 블로그나 코드 산출물을 억지로 요구하지 않는다.
- **기존 장기 기술 축**: ① JS/TS/Node.js 코어 + 블로그 → ② CS/OOP/디자인패턴/아키텍처(헥사고날, 클린) → ③ 시스템 디자인 → ④ MySQL. 이는 장기 기술 순서이며 2026-07-22 이후 개인 학습 일정은 아래 수학 우선순위가 덮어쓴다.
- **병렬 축 제한**: 공고, 면접과 주일학교 플랫폼은 필요할 때 유지한다. GraphQL/gRPC/Kafka/Redis/Terraform/k8s와 자격증은 현업에 즉시 필요한 경우만 학습하고, 별도 DevOps 커리큘럼은 [[2027-DevOps-Practical-Roadmap|2027 DevOps 실전 로드맵]]에 따라 2027년 하반기에 시작한다.
- **예외**: 시스템 디자인과 주일학교 플랫폼 적용처럼 바로 실습으로 이어지는 강의는 시청해도 된다. 자격증 강의는 해당 트랙을 gate 뒤에 실제로 시작한 경우에만 허용하며 2026년 수학 단계에는 병행하지 않는다.
...
- **수학 출발점 (2026-07-22 갱신)**: 사용자는 수학 기초가 거의 없다고 자가 진단했다. 기초 연산과 대수부터 시작하며, 기존의 6주 완주 가정이나 교재 진도율로 압박하지 않는다. 별도 퀴즈보다 손계산, 풀이 설명과 작은 코드 검산으로 확인한다.
- **2026-2027 현재 순서**: 2026 하반기는 [[2026-H2-Math-Roadmap|수학 완전 기초 로드맵]]을 주축으로 하고 검색과 추천은 업무에 필요한 만큼만 연결한다. 수학 2단계 gate 뒤 2027년에는 [[2027-Search-Recommendation-Roadmap|검색 엔진 baseline과 품질 평가]]를 먼저 통과한 뒤 추천 baseline과 taxonomy 실습으로 전환한다. DevOps는 검색/추천의 1차 성공선이나 전체 학습 경로 완료에 종속시키지 않고 [[2027-DevOps-Practical-Roadmap|별도 실전 로드맵]]으로 2027년 하반기에 재개한다. 코딩 테스트는 제외하며 날짜보다 검토 가능한 산출물을 우선한다.
- **공통 시간 상한과 중단 규칙**: 병렬 트랙의 시간은 기존 학습 시간에 더하지 않는다. 새 트랙 시작 전 최근 4주의 실제 개인 학습 시간 중앙값을 주간 총상한으로 정하고 그 안에서 시간을 재배분한다. 기록이 없다면 주축 학습은 기존 총시간을 늘리지 않은 채 4주를 기록하고, 병렬 트랙은 총상한이 생긴 뒤에만 시작한다. 수학, 검색 또는 추천의 계획 산출물이 2주 연속 밀리면 DevOps를 4주 중단하며, 기존 트랙이 4주 연속 계획대로 유지된 뒤에만 DevOps를 주 2시간에서 최대 3시간으로 늘린다.

## 시간대별 학습 모드

- **퇴근 후**: 집중, 시간 귀함 → 기술 학습은 아웃풋(블로그, 면접 답변, 주일학교 플랫폼 개발), 수학 기초는 개념 이해와 손계산
- **근무 중 짜투리**: 짧고 집중력 애매 → 짧은 강의나 책 복습. 새 수학 개념보다 이미 푼 문제와 용어 복습에 사용
- **How to apply**: "강의 vs 블로그" 단순 충돌로 보지 말 것. 기술 강의는 **"짜투리 적합도"**(짧은 챕터, 훑기, 복습용)를 기준으로 필터하고 깊이 집중이 필요한 주제는 퇴근 후 아웃풋 트랙으로 돌린다. 수학 완전 기초는 블로그보다 손으로 풀이 과정을 남기는 것을 아웃풋으로 인정한다.
