# 면접 준비 내용
면접 준비 내용을 정리합니다.

## 목차

### 기초(Fundamentals)
- [[tech/computer-science/CS&프로그래밍(CS&Programming)|CS&프로그래밍(CS&Programming)]] (10/11)
- [[tech/web/웹&네트워크(Web&Network)|웹&네트워크(Web&Network)]] (2/10)
- [[tech/os-runtime/OS&런타임(OS&Runtime)|OS&런타임(OS&Runtime)]] (21/28)

### 데이터(Data)
- [[tech/database/데이터&저장소(Data&Storage)|데이터&저장소(Data&Storage)]] (20/27)
- [[tech/messaging-data-pipeline/메시징&파이프라인(Messaging&Pipeline)|메시징&파이프라인(Messaging&Pipeline)]] (7/19)

### 설계&성능(Design&Performance)
- [[tech/architecture-design/아키텍처&설계(Architecture&Design)|아키텍처&설계(Architecture&Design)]] (2/11)
- [[tech/performance-scalability/성능&확장성(Performance&Scalability)|성능&확장성(Performance&Scalability)]] (0/14)

### 운영&인프라(Operations&Infrastructure)
- [[tech/infrastructure-cloud/인프라&클라우드(Infrastructure&Cloud)|인프라&클라우드(Infrastructure&Cloud)]] (2/21)
- [[tech/ci-cd/CICD&배포(CICD&Delivery)|CICD&배포(CICD&Delivery)]] (0/12)
- [[tech/observability/관측가능성(Observability)|관측가능성(Observability)]] (0/14)
- [[tech/reliability/안정성엔지니어링(Reliability)|안정성엔지니어링(Reliability)]] (0/10)

### 품질&보안(Quality&Security)
- [[tech/testing-quality/테스트&품질(Testing&Quality)|테스트&품질(Testing&Quality)]] (3/9)
- [[tech/security/보안(Security)|보안(Security)]] (5/16)

### 기타(Others)
- [[tech/fin-ops/비용&운영(FinOps)|비용&운영(FinOps)]] (0/8)
- [[tech/senior/시니어역량(SeniorEngineer)|시니어역량(SeniorEngineer)]] (6/11)
- [[fit/FIT|Interview Fit]]

---

## 세미나 & 강의 정리

### 취업 & 이직 가이드
- [[fit/Resume-Portfolio-Guide|이력서 & 포트폴리오 가이드]] — 지원서 작성, 이력서 포맷, 포트폴리오, 합격/탈락 사례
- [[fit/Job-Search-Strategy|채용공고 분석 & 이직 전략]] — 채용공고 분석, 회사 찾기, 채용 프로세스, 이직 시기/팁
- [[fit/Coding-Test-Strategy|코딩테스트 준비 전략]] — 단계별 접근, 분류별 필수 지식, 복기 방법

### 채용시장 & 성장
- [[fit/Developer-Hiring-Insights|채용인사이트]] — 기업이 원하는 역량, 개발자 역량, 스터디/프로젝트 자산
- [[fit/Growth-Principles|성장의 7가지 원칙]] — 반복, 실전, 회고, 실험, 자기효능감, 습관화, 소셜화, 계단식 성장
- [[fit/Zerobase-Career-Seminar|제로베이스 취업설명회]] — 전자공시 활용, 자소서 정량화, 하향지원 전략

### 도메인별 세미나
- [[fit/SSG-Ecommerce-Seminar|SSG 이커머스]] — 이커머스 도메인 구조, 면접 요구 수준, 기술 스택
- [[fit/Zigzag-Fashion-Seminar|지그재그 패션]] — 패션 산업 구조, 커머스 구간별 핵심 기술
- [[fit/Fintech-Seminar|금융 도메인]] — 여신/수신/이체, 핀테크 특수성, 트래픽 처리 방식
- [[fit/Kakao-Ent-Seminar|카카오엔터]] — 백엔드 인프라 전체 그림, DevOps, AI 활용
- [[fit/11st-Engineer-Seminar|11번가 엔지니어]] — 테스트 전략, 코드리뷰 Pn룰, 엔지니어링 문화
- [[fit/Karrot-Realestate-Seminar|당근 부동산]] — 실행 중심 문화, 가설 기반 개선 사이클, A-Z 경험 중시
- [[fit/Toss-Runners-High-Seminar|토스 러너스하이]] — 내부 평가 기준 5개 축, 채용 철학, 결과로 말하기

### 기술 세미나
- [[tech/web/HTTP-Seminar|HTTP 세미나]] — HTTP 버전별 진화와 핵심 요소
- [[tech/security/FIDO-Seminar|FIDO 세미나]] — FIDO & 패스키(Passkey)

### 밋업
- [[fit/TS-Backend-Meetup-1|TS 백엔드 밋업 1회]] — 스타트업 생존기, 패스워드 해싱, 로그 비용 개선기
- [[fit/TS-Backend-Meetup-2|TS 백엔드 밋업 2회]] — AWSome IaC, 웹소켓, MSA 동시성 프로그래밍
- [[fit/TS-Backend-Meetup-3|TS 백엔드 밋업 3회]] — DDD, 아임웹 MSA 전환기, NestJS DI Deep Dive
